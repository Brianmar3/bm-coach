  ...
  return <main className="grid min-h-[100dvh] place-items-center overflow-y-auto bg-black p-4 text-white">
    <section className="w-full max-w-md rounded-3xl border border-yellow-400/25 bg-zinc-900 p-5 shadow-2xl shadow-black sm:p-7">
      ...
      <form onSubmit={submit} className="mt-6 space-y-4">
        <label className="block text-sm">Usuario<input required autoComplete="username" value={username} onChange={(event) => setUsername(event.target.value)} className="mt-1 w-full rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3 outline-none focus:border-yellow-400" /></label>
        <label className="block text-sm">Contraseña<input required type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} className="mt-1 w-full rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3 outline-none focus:border-yellow-400" /></label>
        <button disabled={loading} className="w-full rounded-xl bg-yellow-400 px-4 py-3 font-bold text-zinc-950 disabled:opacity-60">{loading ? "Ingresando…" : "Iniciar sesión"}</button>
      </form>
    </section>
  </main>;
}
