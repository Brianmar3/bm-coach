...
export function SelfServiceRoutineWizard({ initial }: Props) {
  ...
  async function activate() {
    ...
    try {
      ...
      router.replace("/portal/autogestion/rutina");
      router.refresh();
  ...
  return <div className="mx-auto max-w-3xl">
    <header className="mb-4"><p className="text-[10px] font-black uppercase tracking-[.2em] text-yellow-400">Crear mi rutina</p><div className="mt-2 flex items-center justify-between gap-3"><h1 className="text-2xl font-black sm:text-3xl">{stepTitles[step]}</h1><span className="shrink-0 text-xs text-zinc-500">{step + 1} de 7</span></div><div className="mt-3 grid grid-cols-7 gap-1">{stepTitles.map((title, index) => <span key={title} className={`h-1 rounded-full ${index <= step ? "bg-yellow-400" : "bg-zinc-800"}`} />)}</div></header>
    <section className="rounded-[22px] border border-yellow-400/25 bg-[linear-gradient(145deg,#151515,#090909)] p-4 shadow-[0_16px_36px_rgba(0,0,0,.3)] sm:p-6">
      {step === 0 && <OptionGrid values={SELF_SERVICE_GOALS} selected={answers.objective} choose={(value) => update("objective", value)} />}
      {step === 1 && <OptionGrid values={levels} selected={answers.level} choose={(value) => update("level", value)} />}
      {step === 2 && <div className="grid grid-cols-5 gap-2">{[1,2,3,4,5].map((value) => <button type="button" key={value} onClick={() => update("daysPerWeek", value)} className={optionClass(answers.daysPerWeek === value)}><span className="block text-center text-lg font-black">{value}</span></button>)}</div>}
      {step === 3 && <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{[30,45,60,75].map((value) => <button type="button" key={value} onClick={() => update("sessionMinutes", value)} className={optionClass(answers.sessionMinutes === value)}><BmTimerIcon size={18} className="mb-1 text-yellow-400" />{value} min</button>)}</div>}
      {step === 4 && <div className="space-y-5"><div><h2 className="mb-2 text-xs font-black uppercase tracking-wider text-yellow-400">Lugar</h2><OptionGrid values={TRAINING_LOCATIONS} selected={answers.trainingLocation} choose={(value) => update("trainingLocation", value)} /></div><div><h2 className="mb-2 text-xs font-black uppercase tracking-wider text-yellow-400">Equipamiento disponible</h2><div className="grid grid-cols-2 gap-2">{EQUIPMENT_OPTIONS.map((item) => <button type="button" key={item} onClick={() => toggleEquipment(item)} className={optionClass(answers.equipment.includes(item))}>{item}</button>)}</div></div></div>}
      {step === 5 && <div><p className="mb-3 text-sm text-zinc-400">Es opcional. Si no elegís una, la estructura será equilibrada.</p><OptionGrid values={priorities} selected={answers.priorityMuscle} choose={(value) => update("priorityMuscle", value)} labels={{ "": "Sin prioridad específica" }} /></div>}
      {step === 6 && proposal && <Proposal proposal={proposal} loading={loading} activating={activating} change={setProposal} changeExercise={(dayIndex, exerciseIndex) => void changeExercise(dayIndex, exerciseIndex)} activate={() => void activate()} regenerate={() => void generate((answers.variation ?? 0) + 1)} />}
      {error && <p role="alert" className="mt-4 rounded-xl border border-red-400/25 bg-red-400/10 p-3 text-sm text-red-200">{error}</p>}
      {step < 6 && <div className="mt-6 flex gap-2"><button type="button" onClick={() => setStep((current) => Math.max(0, current - 1))} disabled={step === 0 || loading} className="grid min-h-12 min-w-12 place-items-center rounded-xl border border-white/10 text-zinc-300 disabled:opacity-30" aria-label="Paso anterior"><BmBackIcon size={20} /></button><button type="button" onClick={next} disabled={!canContinue || loading} className="min-h-12 flex-1 rounded-xl bg-yellow-400 px-4 font-black text-black disabled:opacity-50">{loading ? "Generando…" : step === 5 ? "Generar propuesta" : "Confirmar y continuar"}</button></div>}
    </section>
  </div>;
}
