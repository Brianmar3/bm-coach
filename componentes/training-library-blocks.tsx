...
const typeLabels: Record<TrainingBlockType, string> = { STRENGTH: "Fuerza", ROUNDS: "Circuito", INTERVAL: "Intervalos", EMOM: "EMOM", AMRAP: "AMRAP", FOR_TIME: "For time", FREE: "Bloque libre", MOBILITY: "Movilidad" };
...
export function TrainingLibraryBlocksPanel({ blocks, folders, ready, loadError = "", retry, onNew, onEdit, onBlockChanged, onBlockDeleted, onFoldersChanged }: {
  ...
  const [deleteTarget, setDeleteTarget] = useState<TrainingLibraryBlock | null>(null);
  ...
  async function deleteBlock() {
    if (!deleteTarget || statusBusy) return;
    setStatusBusy(deleteTarget.id); setStatusError("");
    try {
      const response = await fetch(`/api/training-library/blocks/${deleteTarget.id}`, { method: "DELETE" });
      if (!response.ok) return setStatusError(await responseError(response, "No se pudo eliminar el bloque."));
      onBlockDeleted(deleteTarget.id);
      setDeleteTarget(null);
    } catch { setStatusError("No se pudo eliminar el bloque."); }
    finally { setStatusBusy(""); }
  }
...
function DeleteBlockDialog({ block, busy, cancel, confirm }: { block: TrainingLibraryBlock; busy: boolean; cancel: () => void; confirm: () => void }) {
  ...
  return <div className="fixed inset-0 z-[80] flex items-end bg-black/80 sm:items-center sm:justify-center sm:p-4" onPointerDown={(event) => { if (event.target === event.currentTarget && !busy) cancel(); }}><section role="dialog" aria-modal="true" aria-labelledby="delete-library-block-title" className="w-full rounded-t-3xl border border-red-400/20 bg-[#121212] p-5 shadow-2xl sm:max-w-lg sm:rounded-2xl sm:p-6"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-red-300">Acción permanente</p><h2 id="delete-library-block-title" className="mt-2 text-xl font-black">Eliminar bloque</h2><p className="mt-3 text-sm text-zinc-300">¿Querés eliminar definitivamente “{block.name}” de la Biblioteca?</p><p className="mt-2 text-sm text-zinc-500">Esta acción no se puede deshacer. Las rutinas que ya usaron una copia del bloque no se modificarán.</p><div className="mt-6 flex justify-end gap-3"><button type="button" disabled={busy} onClick={cancel} className="min-h-11 rounded-xl border border-zinc-700 px-4 text-sm font-bold text-zinc-300 disabled:opacity-50">Cancelar</button><button type="button" disabled={busy} onClick={confirm} className="min-h-11 rounded-xl border border-red-400/30 bg-red-400/10 px-4 text-sm font-black text-red-200 disabled:opacity-50">{busy ? "Eliminando…" : "Eliminar"}</button></div></section></div>;
}
