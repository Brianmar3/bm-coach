...
import { filterTrainingLibraryBlocks } from "@/lib/training-library";
import type { TrainingBlockType } from "@/types/gestion";
import type { TrainingLibraryBlock, TrainingLibraryFolder, TrainingLibraryStatus } from "@/types/training-library";
...
export function TrainingLibraryBlocksPanel({ blocks, folders, ready, onNew, onEdit, onBlockChanged, onFoldersChanged }: {
  ...
  const visible = useMemo(() => filterTrainingLibraryBlocks(blocks, { query, folderId, type, tag, status }), [blocks, folderId, query, status, tag, type]);
  ...
  return <>
    <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div className="flex rounded-xl border border-zinc-800 bg-[#111] p-1"><button type="button" onClick={() => setStatus("active")} className={`min-h-9 rounded-lg px-3 text-xs font-bold ${status === "active" ? "bg-yellow-400 text-zinc-950" : "text-zinc-400"}`}>Activos</button><button type="button" onClick={() => setStatus("archived")} className={`min-h-9 rounded-lg px-3 text-xs font-bold ${status === "archived" ? "bg-yellow-400 text-zinc-950" : "text-zinc-400"}`}>Archivados</button></div><div className="flex flex-wrap justify-end gap-2"><button type="button" onClick={() => setFolderManagerOpen(true)} className="min-h-10 rounded-xl border border-zinc-700 px-3 text-xs font-bold text-zinc-300">Gestionar carpetas</button><button type="button" onClick={onNew} className="min-h-10 rounded-xl border border-yellow-400/30 px-3 text-sm font-bold text-yellow-300">+ Nuevo bloque</button></div></div>
    ...
    {!ready ? <p className="rounded-2xl border border-zinc-800 bg-zinc-900 p-12 text-center text-zinc-500">Cargando bloques…</p> : visible.length === 0 ? <p className="rounded-2xl border border-dashed border-zinc-700 p-12 text-center text-zinc-500">{status === "active" ? "No hay bloques activos que coincidan." : "No hay bloques archivados que coincidan."}</p> : <section className="grid gap-3 md:grid-cols-2 2xl:grid-cols-3">{visible.map((block) => <article key={block.id} className="min-w-0 rounded-2xl border border-white/[.07] bg-[#111] p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className="inline-flex rounded-lg bg-yellow-400/[.09] px-2 py-1 text-[10px] font-black uppercase tracking-wide text-yellow-300">{typeLabels[block.type]}</span><h3 className="mt-2 truncate text-base font-black">{block.name}</h3><p className="mt-1 text-xs text-zinc-500">{block.folder?.name ?? "Sin carpeta"}</p></div><button type="button" onClick={() => onEdit(block)} className="min-h-9 shrink-0 rounded-lg border border-yellow-400/25 px-3 text-xs font-bold text-zinc-100">Abrir</button></div><div className="mt-3 flex flex-wrap gap-1.5">{block.tags.slice(0, 4).map((value) => <span key={value} className="rounded-full border border-zinc-700 px-2 py-1 text-[10px] text-zinc-400">{value}</span>)}{block.tags.length > 4 && <span className="px-1 py-1 text-[10px] text-zinc-500">+{block.tags.length - 4}</span>}</div><div className="mt-4 flex items-end justify-between gap-3 border-t border-zinc-800 pt-3"><div><p className="text-xs text-zinc-300">{block.content.exercises.length} ejercicios{block.content.durationSeconds ? ` · ${Math.round(block.content.durationSeconds / 60)} min` : ""}</p><p className="mt-1 text-[10px] text-zinc-500">Modificado {showDate(block.updatedAt)}</p></div><button type="button" disabled={statusBusy === block.id} onClick={() => void changeStatus(block)} className="min-h-9 rounded-lg px-2 text-xs font-semibold text-zinc-400 disabled:opacity-50">{block.status === "active" ? "Archivar" : "Restaurar"}</button></div></article>)}</section>}
