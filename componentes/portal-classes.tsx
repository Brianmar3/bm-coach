...
export function PortalClasses({ compact = false }: { compact?: boolean }) {
  ...
  async function saveStrength(status: "DRAFT" | "COMPLETED", value: ClassStrengthEditorValue) {
    ...
    const body = await response.json() as { error?: string; message?: string; achievements?: PortalAchievement[]; newAchievements?: CelebrationAchievement[] };
  ...
  if (!data && !error) return <div className="h-44 animate-pulse rounded-2xl bg-zinc-900" />;
  if (compact) return <section className="h-full rounded-2xl border border-yellow-400/15 bg-[linear-gradient(145deg,#181818,#0a0a0a)] p-4 shadow-[0_14px_35px_rgba(0,0,0,.28)] sm:p-5">
    <div className="flex items-start justify-between gap-3"><div><p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.18em] text-yellow-400"><span aria-hidden="true">▣</span>Clases de hoy</p><h2 className="mt-2 text-xl font-black">{todayItems.length ? `${todayItems.length} ${todayItems.length === 1 ? "clase disponible" : "clases disponibles"}` : "Sin clases hoy"}</h2></div><Link href="/portal/clases" className="shrink-0 rounded-lg border border-zinc-700 px-3 py-2 text-xs font-bold text-yellow-300 transition hover:border-yellow-400/40">Ver horarios</Link></div>
    {todayItems.length ? <div className="mt-4 space-y-3">{todayItems.slice(0, 2).map((item) => <article key={item.id} className="rounded-xl border border-zinc-800 bg-black/45 p-3.5"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate font-semibold">{item.name}</p><p className="mt-1 text-xs text-zinc-400">{item.startTime} · {item.category}</p></div><span className="shrink-0 rounded-full border border-zinc-700 bg-zinc-900 px-2 py-1 text-[10px] text-zinc-400">{item.statusLabel}</span></div><p className="mt-2 text-xs text-zinc-500">{item.response === "GOING" ? "Confirmaste que asistirás" : item.response === "NOT_GOING" ? "Indicaste que no asistirás" : "Todavía no respondiste"}</p>{item.canRespond && <ResponseButtons item={item} saving={savingId === item.id} respond={respond} />}</article>)}</div> : <div className="mt-4 rounded-xl border border-dashed border-zinc-800 bg-black/25 p-5"><p className="text-sm text-zinc-400">Hoy no hay clases programadas.</p><p className="mt-1 text-xs text-zinc-600">Podés consultar tus horarios semanales.</p></div>}
