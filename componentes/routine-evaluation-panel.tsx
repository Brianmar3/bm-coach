...
export function RoutineEvaluationPanel({ student, context, loading }: { student: Student | undefined; context: LatestEvaluationContext | null; loading: boolean }) {
  ...
  if (loading) return <section className="mt-4 rounded-xl border border-zinc-800 bg-zinc-950/60 p-3 text-xs text-zinc-500">Cargando evaluación del alumno…</section>;
  if (!context?.evaluation) return <section className="mt-4 rounded-xl border border-zinc-800 bg-zinc-950/60 p-3"><p className="text-xs font-bold uppercase tracking-wider text-yellow-400">Evaluación del alumno</p><p className="mt-1 text-xs text-zinc-400">Sin evaluación disponible. Esto no bloquea la creación de la rutina.</p></section>;
  const { evaluation, interpretation } = context; const visibleAlerts = interpretation.alerts.filter((item) => !dismissed.includes(item.id)).slice(0, 2);
  return <><section className="mt-4 rounded-xl border border-yellow-400/20 bg-yellow-400/[.03] p-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-yellow-400">Evaluación del alumno</p><p className="mt-1 text-sm font-bold">{evaluation.primaryGoal || "Objetivo no informado"}</p><p className="mt-0.5 text-xs text-zinc-500">{date(evaluation.date)} · {validity[interpretation.validity]}</p></div><button type="button" onClick={() => setDetailsOpen(true)} className="min-h-9 rounded-lg border border-yellow-400/30 px-3 text-xs font-bold text-yellow-300">Ver evaluación completa</button></div>{interpretation.priorities.length > 0 && <div className="mt-2 flex flex-wrap gap-1.5">{interpretation.priorities.slice(0, 3).map((item) => <span key={item.id} className="rounded-full bg-zinc-800 px-2 py-1 text-[11px] text-zinc-300">{item.message}</span>)}</div>}{visibleAlerts.map((alert) => <div key={alert.id} className="mt-2 flex items-start justify-between gap-3 rounded-lg bg-orange-400/10 p-2 text-xs text-orange-200"><span>{alert.message}</span><button type="button" onClick={() => setDismissed((items) => [...items, alert.id])} className="shrink-0 text-zinc-400">Ignorar</button></div>)}{evaluation.reassessmentDate && <p className="mt-2 text-xs text-zinc-500">Reevaluación sugerida: {date(evaluation.reassessmentDate)}</p>}</section>{detailsOpen && <EvaluationPlanningDetails context={context} close={() => setDetailsOpen(false)}/>}</>;
}
