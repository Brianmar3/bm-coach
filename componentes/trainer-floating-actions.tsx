  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[60] flex items-end bg-black/70 backdrop-blur-sm md:items-center md:justify-center md:p-6" onPointerDown={() => close()}>
      <section id={dialogId} role="dialog" aria-modal="true" aria-labelledby={`${dialogId}-title`} className="w-full rounded-t-3xl border border-yellow-400/15 bg-zinc-950 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] shadow-2xl md:max-w-lg md:rounded-3xl md:p-5" onPointerDown={(event) => event.stopPropagation()}>
        ...
        <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-yellow-400">Acciones rápidas</p><h2 id={`${dialogId}-title`} className="mt-1 text-lg font-bold">{title}</h2><p className="mt-1 text-xs text-zinc-500">{description}</p></div><button type="button" onClick={() => close()} className="grid h-10 w-10 place-items-center rounded-xl text-xl text-zinc-400 hover:bg-zinc-800" aria-label="Cerrar acciones rápidas">×</button></div>
