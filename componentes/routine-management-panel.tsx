...
export function RoutineManagementPanel({ routines, mode, ready, busyId, duplicatingId, actions }: { routines: TrainingRoutine[]; mode: Mode; ready: boolean; busyId: string; duplicatingId: string; actions: Actions }) {
  ...
  if (!routines.length) return <p className="rounded-2xl border border-dashed border-zinc-700 p-12 text-center text-zinc-500">No hay {mode === "plantillas" ? "plantillas" : "rutinas"} que coincidan con los filtros.</p>;
