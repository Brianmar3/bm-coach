...
export function RoutineManagementPanel({ routines, mode, ready, busyId, duplicatingId, actions }: { routines: TrainingRoutine[]; mode: Mode; ready: boolean; busyId: string; duplicatingId: string; actions: Actions }) {
  ...
  if (!routines.length) return <p className="rounded-2xl border border-dashed border-zinc-700 p-12 text-center text-zinc-500">No hay {mode === "plantillas" ? "plantillas" : mode === "asignaciones" ? "asignaciones" : "rutinas"} que coincidan con los filtros.</p>;
  return <div className={`grid min-w-0 gap-3 ${selected ? "xl:grid-cols-[minmax(0,1fr)_340px]" : "grid-cols-1"}`}>
    <section className="min-w-0 overflow-visible rounded-2xl border border-white/[.07] bg-[#111]">
      <div className="hidden grid-cols-[minmax(140px,1.35fr)_minmax(110px,.9fr)_minmax(85px,.7fr)_70px_80px_90px_75px_112px] gap-3 border-b border-zinc-800 px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-zinc-500 lg:grid"><span>{mode === "asignaciones" ? "Alumno" : mode === "plantillas" ? "Plantilla" : "Rutina"}</span><span>{mode === "asignaciones" ? "Rutina" : "Alumno"}</span><span>Objetivo</span><span>Estado</span><span>Volumen</span><span>Última sesión</span><span>Progreso</span><span className="text-right">Acciones</span></div>
