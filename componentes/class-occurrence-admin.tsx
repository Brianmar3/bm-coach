...
export function ClassOccurrenceAdmin() {
  ...
  return <section className="mb-7 rounded-2xl border border-zinc-800 bg-zinc-900">
    ...
    {loading ? <p className="p-8 text-center text-zinc-500">Cargando clases…</p> : items.length ? <div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">{items.map((item) => <button key={item.id} onClick={() => setSelectedId(item.id)} className="rounded-xl border border-zinc-800 bg-zinc-950 p-4 text-left hover:border-yellow-400/50"><div className="flex justify-between gap-3"><div><p className="font-bold">{item.name}</p><p className="mt-1 text-sm text-zinc-400">{item.startTime}–{item.endTime}</p></div><span className="text-xs text-zinc-500">{item.statusLabel}</span></div><p className="mt-4 text-sm text-emerald-300">{item.confirmedCount} confirmados</p><p className="mt-1 text-xs text-zinc-500">{item.capacity === null ? "Sin límite" : `${Math.max(item.capacity - item.confirmedCount, 0)} lugares disponibles`} · {item.noResponse.length} sin respuesta</p><div className="mt-3 flex flex-wrap gap-1">{item.students.filter((student) => student.response === "GOING").slice(0, 4).map((student) => <span key={student.id} className="rounded-full bg-zinc-900 px-2 py-1 text-xs">{student.name}</span>)}</div><p className="mt-4 text-sm font-bold text-yellow-400">Ver detalle →</p></button>)}</div> : <p className="p-8 text-center text-zinc-500">No hay clases programadas para esta fecha.</p>}
