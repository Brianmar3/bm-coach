...
export function ClassOccurrenceAdmin() {
  ...
  return <section className="mb-7 rounded-2xl border border-zinc-800 bg-zinc-900">
    ...
    {loading ? <p className="p-8 text-center text-zinc-500">Cargando clases…</p> : items.length ? <div className="grid gap-2 p-3 sm:grid-cols-2 xl:grid-cols-3">{items.map((item) => <OccurrenceCard key={item.id} item={item} open={() => setSelectedId(item.id)} />)}</div> : <p className="p-8 text-center text-zinc-500">No hay clases programadas para esta fecha.</p>}
...
function OccurrenceCard({ item, open }: { item: AdminClassOccurrence; open: () => void }) {
  const attendanceHref = item.scheduleId ? `/asistencias?scheduleId=${encodeURIComponent(item.scheduleId)}&date=${item.date}` : "";
  return <article className="rounded-xl border border-zinc-800 bg-zinc-950/80 p-3">
    <div className="flex justify-between gap-3"><div className="min-w-0"><p className="truncate font-bold">{item.name}</p><p className="mt-0.5 text-sm font-semibold text-yellow-300">{item.startTime}–{item.endTime}</p></div><button type="button" onClick={open} aria-label={`Configuración de ${item.name}`} className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-zinc-700 text-lg text-zinc-400">⋯</button></div>
    <p className="mt-2 text-xs text-zinc-400">{item.confirmedCount} confirmados · {item.noResponse.length} sin respuesta</p>
    <div className="mt-3 flex items-center justify-between gap-2 border-t border-zinc-800 pt-2.5">{attendanceHref ? <Link href={attendanceHref} className="min-h-10 flex-1 rounded-lg border border-emerald-400/30 px-3 py-2 text-center text-xs font-bold text-emerald-300">Ver detalle →</Link> : <span className="text-xs text-zinc-600">Sin horario asociado</span>}<Link href={`/asistencias?view=history&scheduleId=${encodeURIComponent(item.scheduleId ?? "")}&date=${item.date}`} className="min-h-10 rounded-lg px-3 py-2 text-xs font-semibold text-zinc-400">Historial</Link></div>
  </article>;
}
