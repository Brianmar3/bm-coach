...
export function StudentInvitations({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [serviceType, setServiceType] = useState<StudentServiceType | "">("");
  function close() { setCreatedUrl(""); setServiceType(""); setError(""); onClose(); }
  async function load() {
  ...
  async function create() {
    if (busy || !serviceType) return; setBusy(true); setError("");
    try {
      const response = await fetch("/api/alumnos/invitaciones", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ serviceType }) });
