...
export function PlatformTrainers() {
  ...
  return <>
    ...
    <div className="mb-5 space-y-3"><label className="flex min-h-11 max-w-xl items-center gap-3 rounded-xl border border-zinc-800 bg-zinc-900 px-4 focus-within:border-yellow-400"><span aria-hidden="true" className="text-zinc-500">⌕</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por nombre, email, workspace o plan" className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-zinc-600" /></label><div className="flex flex-wrap gap-2" aria-label="Filtrar por membresía">{filters.map(([value, label]) => <button key={value} type="button" onClick={() => setFilter(value)} className={`rounded-xl px-3 py-2 text-xs font-bold ${filter === value ? "bg-yellow-400 text-zinc-950" : "bg-zinc-900 text-zinc-400"}`}>{label}</button>)}</div></div>
    {error && <p role="alert" className="mb-4 rounded-xl border border-red-400/20 bg-red-400/10 p-3 text-sm text-red-200">{error}</p>}
    <section className="space-y-3">{loading ? <p className="text-zinc-500">Cargando…</p> : visible.map((trainer) => { const workspace = trainer.memberships[0]?.workspace; const membership = trainer.trainerSubscription; const effective = trainer.effectiveSubscriptionStatus; return <article key={trainer.id} className="rounded-2xl border border-zinc-800 bg-zinc-900/80 p-4 md:grid md:grid-cols-[minmax(190px,1.35fr)_minmax(150px,1fr)_minmax(135px,.8fr)_110px_72px_auto] md:items-center md:gap-4"><div className="min-w-0"><div className="flex items-start justify-between gap-3 md:block"><div className="min-w-0"><h2 className="truncate font-bold">{trainer.name}</h2><p className="truncate text-sm text-zinc-400">{trainer.email}</p></div>{trainer.status === "SUSPENDED" && <span className="shrink-0 rounded-full bg-zinc-700 px-2 py-1 text-[10px] font-bold text-zinc-300 md:mt-2 md:inline-block">ACCESO SUSPENDIDO</span>}</div></div><dl className="mt-4 md:mt-0"><dt className="text-[10px] font-bold uppercase tracking-wider text-zinc-600">Marca / workspace</dt><dd className="mt-1 truncate text-sm">{workspace?.name || "—"}</dd></dl><div className="mt-3 flex items-center justify-between gap-3 md:mt-0 md:block"><div><p className="text-[10px] font-bold uppercase tracking-wider text-zinc-600">Plan</p><p className="mt-1 text-sm font-semibold">{membership?.plan ?? "Sin membresía configurada"}</p></div>{effective && <span className={`rounded-full px-2 py-1 text-[10px] font-bold ${subscriptionBadge[effective]}`}>{subscriptionLabel[effective]}</span>}</div><dl className="mt-3 md:mt-0"><dt className="text-[10px] font-bold uppercase tracking-wider text-zinc-600">Próximo vencimiento</dt><dd className="mt-1 text-sm">{showDate(membership?.nextDueAt)}</dd></dl><dl className="mt-3 md:mt-0"><dt className="text-[10px] font-bold uppercase tracking-wider text-zinc-600">Alumnos</dt><dd className="mt-1 text-sm font-bold">{workspace?._count.students ?? 0}</dd></dl><div className="mt-4 flex flex-wrap gap-2 md:mt-0 md:justify-end"><Link href={`/platform/trainers/${trainer.id}`} className="rounded-lg border border-zinc-700 px-3 py-2 text-xs font-bold hover:border-yellow-400">Ver entrenador</Link><button type="button" onClick={() => setManaging(trainer)} className="rounded-lg bg-yellow-400/10 px-3 py-2 text-xs font-bold text-yellow-300">{membership ? "Gestionar membresía" : "Configurar membresía"}</button></div></article>; })}</section>
