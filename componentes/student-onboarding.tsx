...
export function StudentOnboarding({ initial, selfService = false, branding = DEFAULT_WORKSPACE_BRANDING }: { initial: StudentOnboardingData; selfService?: boolean; branding?: WorkspaceBranding }) {
  ...
  return <main className="workspace-brand onboarding-shell" style={workspaceBrandingVariables(branding.accentColor) as CSSProperties}><div className="onboarding-aurora" /><div className="onboarding-phone">
    {step === 0 ? <section className="onboarding-welcome onboarding-enter">
      ...
      {step === 4 && <><div className="onboarding-progress onboarding-progress-complete">{[1, 2, 3, 4].map((item) => <i key={item} className="active" />)}</div><div className="onboarding-success"><BmCheckIcon size={48} /></div><div><h1>Perfil completado</h1><p>Ya podemos adaptar mejor tu<br />seguimiento.</p></div><section className="onboarding-summary"><header><strong>Tu información</strong><button type="button" onClick={() => setStep(1)}>Editar <BmEditIcon size={15} /></button></header><Summary Icon={BmMeasurementsIcon} label="Altura" value={`${form.height} cm`} /><Summary Icon={BmWeightIcon} label="Peso actual" value={`${form.weight} kg`} /><Summary Icon={BmBarbellIcon} label="Objetivo principal" value={form.goal} /><Summary Icon={BmProgressIcon} label="Nivel de experiencia" value={form.experienceLevel} /></section>{error && <p role="alert" className="onboarding-error">{error}</p>}<div className="w-full"><button type="button" disabled={saving} onClick={() => void saveStep(5)} className="onboarding-primary">{saving ? "Guardando…" : `Ir a ${branding.displayName}`} <span>→</span></button><small className="mt-3 block">Disciplina hoy, resultados mañana.</small></div></>}
    </section>}
  </div></main>;
}
