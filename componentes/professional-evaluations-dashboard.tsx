...
export function ProfessionalEvaluationsDashboard() {
  ...
  return <ModuleShell title="Evaluaciones" subtitle="Alumnos personalizados y mixtos">
    ...
    {editor && <EvaluationWizard initial={editor} baseUrl={`/api/admin/alumnos/${editor.studentId}/evaluaciones`} profileHeight={0} birthDate={student?.birthDate ?? ""} allowCompletedEditing onClose={() => void closeEditor()} />}
    <TrainerFloatingActions mode="direct" enabled={!studentId && !editor && !selectedEvaluation && !deleteTarget && !creating} actions={[{ label: "Nueva evaluación", symbol: "+", onSelect: startNewEvaluation }]} />
  </ModuleShell>;
}
