...
export function WorkoutBlockTimer({ block, programmed, persistenceKey, update }: TimerProps) {
  ...
  useEffect(() => {
    ...
    const scheduled = window.setTimeout(() => finish(true), 0);
  ...
  const resting = interval?.segment.kind === "REST" || interval?.segment.kind === "ROUND_REST";
  const currentExercise = resting ? null : configuration.exercises[exerciseIndex] ?? null;
  const nextExercise = interval && interval.nextExerciseIndex !== null ? configuration.exercises[interval.nextExerciseIndex] : null;
  const exerciseTarget = resting ? "Recuperación" : block.exercises[exerciseIndex]?.targetLabel ?? (programmed.workSeconds ? `${programmed.workSeconds} segundos` : "Objetivo libre");
  ...
  return <section aria-label={`Cronómetro ${timer.blockType}`} data-timer-status={timer.status} className="mt-3 min-w-0 touch-manipulation select-none overscroll-contain rounded-2xl border border-yellow-400/25 bg-[radial-gradient(circle_at_top,rgba(250,204,21,.1),transparent_55%),#090909] p-3.5 text-center shadow-inner">
    ...
    {interval && <div className="min-w-0"><div className="rounded-2xl border border-yellow-400/20 bg-zinc-950/90 px-3 py-4"><p className="text-[9px] font-black uppercase tracking-[.2em] text-zinc-500">{resting ? "Fase actual" : "Ejercicio actual"}</p><h3 className="mt-1 break-words text-center text-2xl font-black leading-tight text-white max-[340px]:text-xl">{resting ? "Descanso" : currentExercise?.name ?? "Trabajo"}</h3><p className="mt-2 text-sm font-black text-yellow-300">{exerciseTarget}</p>{!resting && <p className="mt-1 text-xs text-zinc-400">Ejercicio {exerciseIndex + 1} de {block.exercises.length}</p>}</div><p className="mt-2 text-xs font-bold text-zinc-300">Ronda {interval.segment.round} de {programmed.rounds ?? 1}</p><p className="mt-1 truncate text-xs text-zinc-500">Siguiente: {nextExercise?.name ?? (interval.finished ? "Finalizar" : "Descanso")}</p><p className="mt-1 text-[10px] text-zinc-600">Total restante: {formatTimerClock(interval.remainingSeconds)}</p></div>}
