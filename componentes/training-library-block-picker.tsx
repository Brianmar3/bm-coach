...
import type { TrainingLibraryBlock, TrainingLibraryFolder } from "@/types/training-library";
...
export function TrainingLibraryBlockPicker({ blocks, folders, ready, error, busyId, close, add }: {
  ...
  const visible = useMemo(() => filterTrainingLibraryBlocks(blocks, { query, folderId, type, tag, status: "active" }), [blocks, folderId, query, tag, type]);

  useEffect(() => {
    const handleEscape = (event: KeyboardEvent) => { if (event.key === "Escape" && !busyId) { event.preventDefault(); close(); } };
  ...
  return <div className="fixed inset-0 z-[70] flex items-end bg-black/80 sm:items-center sm:justify-center sm:p-4">
    <section role="dialog" aria-modal="true" aria-labelledby="block-library-picker-title" className="flex max-h-[92dvh] w-full min-w-0 flex-col overflow-hidden rounded-t-3xl border border-zinc-700 bg-[#121212] text-white sm:max-w-3xl sm:rounded-2xl">
      ...
      <div className="min-w-0 overflow-y-auto p-4 sm:p-5">
        ...
        {error ? <p role="alert" className="mt-4 rounded-xl border border-red-400/20 bg-red-400/10 p-4 text-sm text-red-200">{error}<span className="mt-1 block text-xs text-zinc-400">Podés cerrar este selector y usar “Crear nuevo”.</span></p> : !ready ? <p className="mt-4 rounded-xl border border-zinc-800 p-8 text-center text-sm text-zinc-500">Cargando bloques…</p> : visible.length === 0 ? <p className="mt-4 rounded-xl border border-dashed border-zinc-700 p-8 text-center text-sm text-zinc-500">No hay bloques activos que coincidan.</p> : <div className="mt-4 space-y-2">{visible.map((block) => <article key={block.id} className="min-w-0 rounded-xl border border-zinc-800 bg-zinc-950/65 p-3"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-md bg-yellow-400/[.09] px-2 py-1 text-[10px] font-black uppercase text-yellow-300">{TRAINING_BLOCK_LABELS[block.type]}</span><span className="truncate text-xs text-zinc-500">{block.folder?.name ?? "Sin carpeta"}</span></div><h3 className="mt-2 break-words text-sm font-black text-zinc-100">{block.name}</h3><p className="mt-1 text-xs text-zinc-400">{blockSummary(block)}</p><div className="mt-2 flex flex-wrap gap-1">{block.tags.slice(0, 4).map((value) => <span key={value} className="rounded-full border border-zinc-700 px-2 py-0.5 text-[10px] text-zinc-400">{value}</span>)}</div></div><button type="button" disabled={Boolean(busyId)} onClick={() => add(block)} className="min-h-10 shrink-0 rounded-lg border border-yellow-400/30 px-3 text-xs font-black text-yellow-300 disabled:opacity-40">{busyId === block.id ? "Agregando…" : "Agregar"}</button></div></article>)}</div>}
      </div>
    </section>
  </div>;
}
