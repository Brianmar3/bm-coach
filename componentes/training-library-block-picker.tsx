...
function exerciseTarget(block: TrainingLibraryBlock, exercise: TrainingLibraryBlock["content"]["exercises"][number]) {
  if (exercise.targetType === "TIME" && exercise.targetSeconds) return `${exercise.targetSeconds} s${exercise.targetSide ? `/${exercise.targetSide}` : ""}`;
  if (exercise.targetType === "REPS" && exercise.targetRepetitions) return `${exercise.targetRepetitions}${exercise.targetSide ? `/${exercise.targetSide}` : ""}`;
  if (exercise.targetType === "DISTANCE" && exercise.targetDistance) return exercise.targetDistance;
  if (exercise.targetType === "REST" && exercise.targetSeconds) return `${exercise.targetSeconds} s de pausa`;
  if (block.type === "STRENGTH") return `${exercise.sets} × ${exercise.repetitions}`;
  return "Objetivo libre";
}

function blockConfiguration(block: TrainingLibraryBlock) {
  const values: string[] = [];
  if (block.content.rounds) values.push(`${block.content.rounds} rondas`);
  if (block.content.durationSeconds) values.push(`${Math.round(block.content.durationSeconds / 60)} min`);
  if (block.content.workSeconds) values.push(`${block.content.workSeconds} s trabajo`);
  if (block.content.restSeconds !== null) values.push(`${block.content.restSeconds} s pausa`);
  if (block.content.restBetweenRoundsSeconds) values.push(`${block.content.restBetweenRoundsSeconds} s entre rondas`);
  if (block.content.targetRounds) values.push(`${block.content.targetRounds} estaciones`);
  return values;
}
  ...
  return <div className="fixed inset-0 z-[70] flex items-end bg-black/80 sm:items-center sm:justify-center sm:p-4">
    <section role="dialog" aria-modal="true" aria-labelledby="block-library-picker-title" className="flex max-h-[92dvh] w-full min-w-0 flex-col overflow-hidden rounded-t-3xl border border-zinc-700 bg-[#121212] text-white sm:max-w-3xl sm:rounded-2xl">
      ...
      <div className="min-w-0 overflow-y-auto p-4 sm:p-5">
        <nav aria-label="Vista rápida de Biblioteca" className="mb-3 grid grid-cols-3 rounded-xl border border-zinc-800 bg-zinc-950/60 p-1">{([['all', 'Todos'], ['favorites', 'Favoritos'], ['recent', 'Recientes']] as const).map(([value, title]) => <button key={value} type="button" aria-current={view === value ? "page" : undefined} onClick={() => { setView(value); setExpandedId(""); }} className={`min-h-10 min-w-0 rounded-lg px-1 text-xs font-bold transition sm:text-sm ${view === value ? "bg-yellow-400/[.12] text-yellow-300" : "text-zinc-400"}`}>{title}</button>)}</nav>
        ...
        {error ? <p role="alert" className="mt-4 rounded-xl border border-red-400/20 bg-red-400/10 p-4 text-sm text-red-200">{error}<span className="mt-1 block text-xs text-zinc-400">Podés cerrar este selector y usar “Crear nuevo”.</span></p> : !ready ? <p className="mt-4 rounded-xl border border-zinc-800 p-8 text-center text-sm text-zinc-500">Cargando bloques…</p> : visible.length === 0 ? <div className="mt-4 rounded-xl border border-dashed border-zinc-700 p-8 text-center text-sm text-zinc-500"><p>{emptyMessage}</p>{view === "favorites" && <p className="mt-2 text-xs">Marcá con ★ los bloques que usás más.</p>}</div> : <div className="mt-4 space-y-2">{visible.map((block) => { const expanded = expandedId === block.id; const previewId = `block-preview-${block.id}`; return <article key={block.id} className="min-w-0 rounded-xl border border-zinc-800 bg-zinc-950/65 p-3"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-md bg-yellow-400/[.09] px-2 py-1 text-[10px] font-black uppercase text-yellow-300">{TRAINING_BLOCK_LABELS[block.type]}</span><span className="truncate text-xs text-zinc-500">{block.folder?.name ?? "Sin carpeta"}</span></div><h3 className="mt-2 break-words text-sm font-black text-zinc-100">{block.name}</h3><p className="mt-1 text-xs text-zinc-400">{blockSummary(block)}</p>{view === "recent" && <p className="mt-1 text-[10px] text-zinc-500">{trainingLibraryLastUsedLabel(block.lastUsedAt)}</p>}<div className="mt-2 flex flex-wrap gap-1">{block.tags.slice(0, 4).map((value) => <span key={value} className="rounded-full border border-zinc-700 px-2 py-0.5 text-[10px] text-zinc-400">{value}</span>)}</div></div><div className="flex shrink-0 gap-1"><TrainingLibraryFavoriteButton block={block} busy={favoriteBusy === block.id} toggle={(item) => void toggleFavorite(item)} /><button type="button" aria-label={`${expanded ? "Ocultar" : "Ver"} contenido de ${block.name}`} aria-expanded={expanded} aria-controls={previewId} onClick={() => setExpandedId(expanded ? "" : block.id)} className="grid min-h-10 min-w-10 place-items-center rounded-lg border border-zinc-700 text-sm text-zinc-300">{expanded ? "⌃" : "⌄"}</button><button type="button" disabled={Boolean(busyId || favoriteBusy)} onClick={() => add(block)} className="min-h-10 rounded-lg border border-yellow-400/30 px-3 text-xs font-black text-yellow-300 disabled:opacity-40">{busyId === block.id ? "Agregando…" : "Agregar"}</button></div></div>{expanded && <div id={previewId} className="mt-3 border-t border-zinc-800 pt-3"><div className="flex flex-wrap gap-1.5">{blockConfiguration(block).map((value) => <span key={value} className="rounded-md bg-zinc-900 px-2 py-1 text-[10px] text-zinc-400">{value}</span>)}</div>{block.content.instructions && <p className="mt-2 text-xs text-zinc-400">{block.content.instructions}</p>}<div className="mt-2 divide-y divide-zinc-800/70 rounded-lg border border-zinc-800">{block.content.exercises.map((exercise) => <div key={`${block.id}-${exercise.order}`} className="grid gap-1 px-3 py-2.5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="min-w-0"><p className="truncate text-xs font-bold text-zinc-200">{exercise.name}</p><p className="mt-0.5 text-[10px] text-zinc-500">{[exercise.equipment, exercise.observations].filter(Boolean).join(" · ")}</p></div><p className="text-xs font-semibold text-yellow-200">{exerciseTarget(block, exercise)}{exercise.restSeconds ? <span className="ml-2 font-normal text-zinc-500">· {exercise.restSeconds} s descanso</span> : null}</p></div>)}</div></div>}</article>; })}</div>}
      </div>
    </section>
  </div>;
}
