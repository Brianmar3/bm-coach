  ...
  return <div className="mx-auto max-w-5xl space-y-2.5 sm:space-y-3">
    <header className="relative overflow-hidden rounded-[24px] border border-white/[.07] bg-[radial-gradient(circle_at_88%_8%,rgba(250,204,21,.045),transparent_28%),linear-gradient(145deg,#151515,#090909_70%)] p-4 shadow-[0_18px_45px_rgba(0,0,0,.34)] sm:p-5">
      <div className={`relative min-h-28 sm:min-h-32 ${groupClassesEnabled && !routineFocused ? "pr-[5.4rem] sm:pr-28" : ""}`}>
        ...
        <p className="mt-1.5 text-xs text-zinc-500 sm:text-sm">{routineFocused ? "Hoy avanzás una parte más de tu plan." : "Vamos por un día más de progreso."}</p>
...
function MonthlyRankingDialog({ onClose, expectedMonthlyPoints }: { onClose: () => void; expectedMonthlyPoints: number }) {
  ...
  return <div role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }} className="fixed inset-0 z-[80] flex items-end justify-center bg-black/75 p-0 backdrop-blur-sm sm:items-center sm:p-5">
    <section role="dialog" aria-modal="true" aria-labelledby="ranking-title" className="portal-ranking-sheet flex w-full flex-col overflow-hidden rounded-t-[24px] border border-white/10 bg-[#111] shadow-2xl sm:max-w-lg sm:rounded-[24px]">
      <header className="sticky top-0 z-20 flex shrink-0 items-start justify-between gap-4 border-b border-white/[.07] bg-[#111] p-4">
        <div><p className="text-[9px] font-black uppercase tracking-[.18em] text-yellow-400">Puntos del mes</p><h2 id="ranking-title" className="mt-1 text-xl font-black">Ranking mensual</h2></div>
        <button type="button" onClick={onClose} aria-label="Cerrar ranking" className="grid size-11 place-items-center rounded-xl border border-white/10 text-xl text-zinc-300 hover:bg-white/[.05] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-300">×</button>
      </header>
      {ranking && <div className="shrink-0 px-4 pt-4">
        <div className="grid grid-cols-2 gap-2 rounded-xl border border-yellow-400/15 bg-yellow-400/[.04] p-3">
          <div><p className="text-[9px] uppercase tracking-[.12em] text-zinc-500">Tu posición</p><strong className="mt-1 block text-xl text-white">{ranking.currentPosition ? `#${ranking.currentPosition}` : "—"}</strong></div>
          <div className="text-right"><p className="text-[9px] uppercase tracking-[.12em] text-zinc-500">Tus puntos del mes</p><strong className="mt-1 block text-xl text-yellow-300">{ranking.currentPoints.toLocaleString("es-AR")}</strong></div>
        </div>
      </div>}
      <div className="portal-ranking-scroll min-h-0 flex-1 overflow-y-auto px-4 pt-3">
        {error ? <p className="rounded-xl border border-red-400/20 bg-red-400/[.06] p-3 text-sm text-red-200">{error}</p> : !ranking ? <div className="space-y-2" aria-label="Cargando ranking"><div className="h-16 animate-pulse rounded-xl bg-zinc-800/60" /><div className="h-16 animate-pulse rounded-xl bg-zinc-800/40" /></div> : <>{ranking.currentPoints !== expectedMonthlyPoints && <p className="mb-3 text-xs text-amber-300">Tus puntos se actualizaron. Cerrá esta vista para refrescar el resumen.</p>}<ol className="space-y-1.5">{ranking.ranking.map((entry, index) => <li key={entry.studentId} className={`flex min-h-14 items-center gap-3 rounded-xl border px-3 py-2 ${entry.studentId === ranking.currentStudentId ? "border-yellow-400/25 bg-yellow-400/[.05]" : "border-white/[.06] bg-black/20"}`}><span className={`grid size-8 shrink-0 place-items-center rounded-full text-xs font-black ${index < 3 ? "bg-yellow-400/10 text-yellow-300" : "bg-zinc-800 text-zinc-400"}`}>{index + 1}</span><span className="min-w-0 flex-1 truncate text-sm font-semibold text-zinc-100">{entry.studentName}</span><strong className="shrink-0 text-sm text-yellow-300">{entry.total.toLocaleString("es-AR")} pts</strong></li>)}</ol><p className="mt-4 text-center text-[11px] leading-relaxed text-zinc-500">El ranking se calcula con los puntos obtenidos durante el mes actual.</p></>}
      </div>
    </section>
  </div>;
}
...
function WorkoutView({ data }: { data: PortalData }) {
  ...
  return <>
    ...
    {draft && <>
      ...
      {hasPersonalizedService(data.profile.serviceType) && <div className="mt-5 border-t border-zinc-800 pt-5"><PortalActionCard href="/portal/progreso" ariaLabel="Ver mi progreso" title="Ver mi progreso" subtitle="Historial, evolución y avances" icon={<svg viewBox="0 0 24 24" className="size-5 fill-none stroke-current" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M4 19V9m5 10V5m5 14v-7m5 7V3" /><path d="m3 15 5-4 5 2 7-7" /></svg>} /></div>}
