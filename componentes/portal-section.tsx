...
export function PortalSection({ section }: { section: Section }) {
  ...
  if (!data) return null;
  if (section === "rutina") return <div className="pb-20 md:pb-0"><WorkoutView data={data} /><div id="historial-entrenamientos" className="mt-8 scroll-mt-24 border-t border-zinc-800 pt-8"><WorkoutHistoryView data={data} /></div></div>;
...
function WorkoutView({ data }: { data: PortalData }) {
  ...
  return <PageHeader title={routineDisplayName} subtitle={`Día ${selectedDay.dayNumber}${selectedDayName ? ` · ${selectedDayName}` : ""}`}>
    ...
    <section className="mb-5 overflow-hidden rounded-2xl border border-yellow-400/15 bg-[linear-gradient(145deg,#1a1a1a,#0d0d0d)] p-4 shadow-[0_14px_32px_rgba(0,0,0,.24)]">
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-lg font-black text-white">{dayFocus}</p><p className="mt-1 text-xs text-zinc-400">{selectedDay.exercises.length} ejercicios{selectedDay.estimatedMinutes ? ` · ${selectedDay.estimatedMinutes} min` : ""}{selectedDay.id === suggestedDayId ? " · Día sugerido" : ""}</p></div><span className={`shrink-0 rounded-full border px-2.5 py-1 text-[10px] font-bold ${completedTotal === totalSets && totalSets ? "border-emerald-400/20 bg-emerald-400/10 text-emerald-300" : started ? "border-yellow-400/20 bg-yellow-400/10 text-yellow-300" : "border-zinc-700 bg-zinc-800 text-zinc-400"}`}>{completedTotal === totalSets && totalSets ? "Completado" : started ? "En curso" : "Sin comenzar"}</span></div>
      <div className="mt-4 flex items-center justify-between gap-3 text-xs"><span className="font-semibold text-zinc-300">{completedExercises}/{selectedDay.exercises.length} ejercicios completados</span><span className="text-zinc-500">{Math.round(dayProgress)}%</span></div>
      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-zinc-800"><div className="h-full rounded-full bg-gradient-to-r from-amber-500 to-yellow-300 transition-[width]" style={{ width: `${dayProgress}%` }} /></div>
    </section>
    ...
    {draft && <>
      <div className="mt-5 space-y-3">{draft.exercises.map((exercise, exerciseIndex) => {
        ...
        return <article key={exercise.exerciseId} className={`overflow-hidden rounded-2xl border bg-zinc-900/90 transition ${open ? "border-yellow-400/25 shadow-[0_12px_28px_rgba(0,0,0,.22)]" : "border-zinc-800"}`}>
          ...
          {open && <div id={`exercise-${exercise.exerciseId}`} className="border-t border-zinc-800 px-3 pb-4 pt-3 sm:px-4">
            ...
            {(programmed?.observations || programmed?.videoUrl) && <details className="mt-3 rounded-xl border border-zinc-800 bg-black/30 px-3 py-2 text-sm text-zinc-400"><summary className="cursor-pointer list-none font-semibold text-yellow-300 outline-none focus-visible:ring-2 focus-visible:ring-yellow-300">Ver técnica</summary><div className="mt-3 border-t border-zinc-800 pt-3">{programmed.observations && <p className="text-xs leading-relaxed text-zinc-300">{programmed.observations}</p>}{programmed.videoUrl && <a href={programmed.videoUrl} target="_blank" rel="noreferrer" className="mt-3 inline-flex min-h-10 items-center rounded-lg border border-yellow-400/30 px-3 text-xs font-bold text-yellow-300">Abrir video técnico</a>}</div></details>}
      ...
      <div className="fixed inset-x-3 bottom-[calc(env(safe-area-inset-bottom)+6rem)] z-30 mx-auto flex max-w-2xl gap-2 rounded-2xl border border-yellow-400/15 bg-black/90 p-2 shadow-[0_16px_40px_rgba(0,0,0,.65)] backdrop-blur-xl md:sticky md:bottom-4 md:mt-6 md:justify-end md:border-zinc-800 md:bg-zinc-950/95"><button type="button" disabled={saving || !started} onClick={() => save(false)} className="min-h-11 min-w-0 flex-1 rounded-xl border border-yellow-400/40 px-3 py-2.5 text-xs font-bold text-yellow-300 outline-none focus-visible:ring-2 focus-visible:ring-yellow-300 disabled:opacity-50 md:flex-none md:text-sm">{savingAction === "draft" ? "Guardando…" : "Guardar progreso"}</button><button type="button" disabled={saving || !started} onClick={openFinalSummary} className="min-h-11 min-w-0 flex-[1.15] rounded-xl bg-yellow-400 px-3 py-2.5 text-xs font-black text-zinc-950 outline-none focus-visible:ring-2 focus-visible:ring-yellow-100 disabled:opacity-50 md:flex-none md:text-sm">Finalizar entrenamiento</button></div>
