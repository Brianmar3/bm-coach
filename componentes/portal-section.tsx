...
function AchievementsSpotlight({ data }: { data: PortalData }) {
  ...
  return <section className="rounded-2xl border border-yellow-400/25 bg-zinc-900 p-4"><div className="flex items-center justify-between gap-3"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-yellow-400">Tus logros</p><span className="rounded-full bg-yellow-400/10 px-2 py-1 text-xs font-bold text-yellow-200">{unlocked.length} obtenidos</span></div>{latest && <div className="mt-3 flex items-start gap-3"><span aria-hidden="true" className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-yellow-400/10 text-xl text-yellow-400">{latest.icon}</span><div><p className="text-xs text-zinc-500">Último logro</p><p className="mt-1 font-bold">{latest.name}</p><p className="mt-1 text-xs text-zinc-500">{date(latest.unlockedAt)}{latest.level ? ` · ${latest.level}` : ""}</p></div></div>}{next && <div className="mt-3 rounded-xl bg-zinc-950 p-3"><div className="flex justify-between gap-3 text-xs"><span><span className="block text-zinc-500">Próximo objetivo</span><strong className="mt-1 block text-zinc-200">{next.name}</strong></span><span className="self-end text-zinc-400">{next.progress} de {next.target}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-zinc-800"><div className="h-full rounded-full bg-yellow-500" style={{ width: `${Math.min(100, next.progress / next.target * 100)}%` }} /></div></div>}</section>;
}
