...
function PortalOverview({ data }: { data: PortalData }) {
  ...
  const classesOnly = data.profile.serviceType === "CLASSES";
  ...
  return <div className={`mx-auto max-w-5xl ${classesOnly ? "space-y-3.5 sm:space-y-4" : "space-y-2.5 sm:space-y-3"}`}>
    <header className={`relative overflow-hidden border bg-[radial-gradient(circle_at_88%_8%,rgba(250,204,21,.055),transparent_30%),linear-gradient(145deg,#151515,#090909_70%)] shadow-[0_18px_45px_rgba(0,0,0,.34)] ${classesOnly ? "rounded-[28px] border-white/[.1] p-5 sm:p-7" : "rounded-[24px] border-white/[.07] p-4 sm:p-5"}`}>
      <div className={`relative ${classesOnly ? "min-h-36 pr-[6.3rem] sm:min-h-40 sm:pr-36" : `min-h-28 sm:min-h-32 ${groupClassesEnabled && !routineFocused ? "pr-[5.4rem] sm:pr-28" : ""}`}`}>
        <p className="flex items-center gap-2 text-[11px] text-zinc-500"><span aria-hidden="true" className={`grid place-items-center text-yellow-400 ${classesOnly ? "size-8 rounded-xl border border-yellow-400/20 bg-yellow-400/[.06]" : ""}`}>▣</span>{todayLabel}</p>
        ...
        <p className={`mt-2 max-w-md leading-relaxed text-zinc-400 ${classesOnly ? "text-sm sm:text-base" : "text-xs sm:text-sm"}`}>{routineFocused ? "Hoy avanzás una parte más de tu plan." : classesOnly ? "Cada clase suma constancia. Seguí construyendo tu mejor versión." : "Vamos por un día más de progreso."}</p>
        {groupClassesEnabled && !routineFocused && <MonthlyAttendanceIndicator data={data} />}
      </div>
    </header>
    <section className={`relative overflow-hidden border bg-[radial-gradient(circle_at_92%_40%,rgba(250,204,21,.05),transparent_32%),linear-gradient(145deg,#141414,#090909)] shadow-[0_12px_28px_rgba(0,0,0,.22)] ${classesOnly ? "rounded-[26px] border-yellow-400/15 px-5 py-5 sm:px-7 sm:py-6" : "rounded-[22px] border-white/[.07] px-4 py-3.5 sm:px-5 sm:py-4"}`}><span aria-hidden="true" className="pointer-events-none absolute -bottom-16 -right-8 h-28 w-52 rotate-[-18deg] rounded-[50%] border-t border-yellow-400/15" /><div className={`relative ${classesOnly ? "flex items-start gap-4" : ""}`}>{classesOnly && <span aria-hidden="true" className="grid size-11 shrink-0 place-items-center rounded-full border border-yellow-400/25 bg-yellow-400/[.06] text-2xl font-black text-yellow-300">“</span>}<div><p className="text-[9px] font-black uppercase tracking-[.2em] text-yellow-400">Enfoque de hoy</p><div className={`flex items-start gap-2.5 ${classesOnly ? "mt-3" : "mt-2"}`}>{!classesOnly && <span aria-hidden="true" className="text-2xl font-black leading-none text-yellow-400/80">“</span>}<div className="min-w-0"><h2 className="line-clamp-2 break-words text-sm font-semibold italic leading-snug text-zinc-100 sm:text-base">{dailyFocus.title}</h2><p className="mt-1 line-clamp-2 break-words text-[11px] leading-relaxed text-zinc-500 sm:text-xs">{dailyFocus.reflection}</p></div></div></div></div></section>
    {homePlan ? <RoutineHomeCard plan={homePlan} /> : groupClassesEnabled && <PortalClasses compact classesOnly={classesOnly} />}
...
function MonthlyAttendanceIndicator({ data }: { data: PortalData }) {
  ...
  return <Link href="/portal/asistencias" aria-label={`Ver detalle de asistencia mensual. ${percentage === null ? "Sin registros este mes." : `${display}. ${detail}.`}`} className="group absolute right-0 top-0 z-10 flex w-[5.75rem] cursor-pointer flex-col items-center gap-1 rounded-2xl transition active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-300 sm:w-[7rem]" title="Ver detalle de asistencias">
    <span className="relative grid h-[78px] w-[78px] place-items-center rounded-full border border-zinc-700/70 shadow-[0_8px_22px_rgba(0,0,0,.28)] transition group-hover:border-yellow-400/40 sm:h-[92px] sm:w-[92px]" style={{ background: `conic-gradient(#facc15 ${angle}deg,#27272a 0deg)` }}><span className="absolute inset-[6px] rounded-full bg-[#0b0b0b]" /><strong className="relative text-lg font-black text-yellow-300 sm:text-2xl">{display}</strong></span>
  ...
  return <>
    ...
    <section className="relative mb-5 overflow-hidden rounded-3xl border border-yellow-400/20 bg-[radial-gradient(circle_at_10%_10%,rgba(250,204,21,.11),transparent_34%),linear-gradient(145deg,#1b1b1b,#0a0a0a)] p-4 shadow-[0_18px_42px_rgba(0,0,0,.34)]">
      ...
      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 pt-7">
        <span aria-hidden="true" className="grid h-12 w-12 place-items-center rounded-2xl border border-yellow-400/20 bg-yellow-400/[.04] text-yellow-300"><svg viewBox="0 0 24 24" className="size-6 fill-none stroke-current" strokeWidth="1.5"><path d="M5 12h14M7 8v8M17 8v8M3.5 10v4M20.5 10v4" /></svg></span>
    ...
    {draft && conditioningBlocks.map((programmed) => {
      ...
      return block ? <WorkoutBlockCard key={block.blockId} block={block} programmed={programmed} libraryMediaEnabled={data.exerciseMediaEnabled} timerPersistenceKey={`${storageKey(selectedDay.id)}:timer:${block.blockId}`} open={openBlockId === block.blockId} toggle={() => setOpenBlockId(openBlockId === block.blockId ? null : block.blockId)} update={(changes) => updateBlockResult(block.blockId, changes)} complete={(changes) => completeBlockResult(block.blockId, changes)} /> : null;
    })}
    {draft && <>
