...
function PortalOverview({ data }: { data: PortalData }) {
  ...
  return <div className="portal-home-sequence mx-auto max-w-5xl space-y-3.5 sm:space-y-4">
    <header className="portal-home-enter portal-home-hero relative overflow-hidden rounded-[26px] border border-yellow-400/25 bg-[radial-gradient(circle_at_86%_12%,rgba(250,204,21,.055),transparent_30%),linear-gradient(145deg,#171717,#090909_72%)] px-5 py-5 shadow-[0_18px_45px_rgba(0,0,0,.34)] min-[390px]:px-6 min-[390px]:py-6 sm:p-8">
      ...
      <div className={`relative min-h-[9.25rem] sm:min-h-44 ${groupClassesEnabled && !routineFocused ? "pr-[6.4rem] min-[390px]:pr-[7.2rem] sm:pr-40" : ""}`}>
        ...
        <h1 className="mt-8 text-[clamp(1.85rem,8vw,3.45rem)] font-black leading-none tracking-[-.045em] text-zinc-50 sm:mt-10">¡Hola, <span className="text-yellow-400">{data.profile.firstName}</span>!</h1>
        <p className="mt-3 max-w-md text-sm leading-relaxed text-zinc-500 sm:text-lg">{routineFocused ? "Hoy avanzás una parte más de tu plan." : "Vamos por un día más de progreso."}</p>
        {groupClassesEnabled && !routineFocused && <MonthlyAttendanceIndicator data={data} />}
      </div>
    </header>
    <section className="portal-home-enter portal-home-focus relative min-h-[10rem] overflow-hidden rounded-[26px] border border-white/[.1] bg-[linear-gradient(145deg,#151515,#090909)] px-5 py-5 shadow-[0_14px_34px_rgba(0,0,0,.28)] min-[390px]:px-6 sm:min-h-[12rem] sm:px-8 sm:py-7"><span aria-hidden="true" className="portal-home-focus-lines" /><div className="relative z-[1]"><p className="text-[10px] font-black uppercase tracking-[.22em] text-yellow-400 sm:text-xs">Enfoque de hoy</p><div className="mt-4 flex items-start gap-3.5 sm:gap-5"><span aria-hidden="true" className="portal-home-focus-quote text-4xl font-black leading-none text-yellow-400/90 sm:text-5xl">“</span><div className="min-w-0 max-w-2xl"><h2 className="break-words text-base font-semibold italic leading-snug text-zinc-100 sm:text-2xl">{dailyFocus.title}</h2><p className="mt-2 break-words text-xs leading-relaxed text-zinc-500 sm:mt-3 sm:text-base">{dailyFocus.reflection}</p></div></div></div></section>
...
function HomeQuickStats({ data, plan }: { data: PortalData; plan: PersonalizedHomePlan | null }) {
  ...
  const cardClass = "portal-home-stat portal-home-interactive group relative min-w-0 overflow-hidden rounded-[20px] border border-yellow-400/30 bg-[linear-gradient(145deg,#151515,#090909)] px-3 py-3.5 shadow-[0_12px_28px_rgba(0,0,0,.25)] transition hover:border-yellow-400/45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-300 min-[390px]:px-4 min-[390px]:py-4 sm:min-h-40 sm:p-5";
  const progressContent = <><BmProgressIcon size={20} className="absolute right-3 top-3 text-yellow-400/60 sm:right-5 sm:top-5" /><p className="pr-5 text-[8px] font-black uppercase tracking-[.15em] text-yellow-400 sm:text-[11px]">{plan ? "Progreso del plan" : "Progreso resumido"}</p><div className="mt-5 flex items-end gap-2"><p className="text-xl font-semibold leading-none text-zinc-100 sm:text-3xl">{progressValue}</p><span aria-hidden="true" className="portal-home-mini-bars flex h-7 items-end gap-0.5"><i className="h-1.5 w-1 bg-yellow-400/30" /><i className="h-3.5 w-1 bg-yellow-400/45" /><i className="h-6 w-1 bg-yellow-400/65" /><i className="h-3 w-1 bg-zinc-700" /></span></div><p className="mt-2 line-clamp-2 text-[9px] leading-snug text-zinc-500 sm:text-xs">{progressLabel}</p></>;
  return <section aria-label="Resumen del alumno" className="portal-home-enter grid grid-cols-3 gap-2 sm:gap-3">
    <Link href="/portal/pagos" className={cardClass}><BmPaymentIcon size={20} className="absolute right-3 top-3 text-yellow-400/60 sm:right-5 sm:top-5" /><p className="pr-5 text-[8px] font-black uppercase tracking-[.15em] text-yellow-400 sm:text-[11px]">Tu cuota</p><p className={`mt-5 line-clamp-2 text-sm font-semibold leading-tight sm:text-2xl ${status.className.includes("emerald") ? "text-emerald-400" : "text-zinc-100"}`}>{status.label}</p><p className="mt-2 line-clamp-2 text-[9px] leading-snug text-zinc-500 sm:text-xs">{dueLabel}</p></Link>
    ...
    <Link href="/portal/puntos" className={`portal-home-points ${cardClass}`}><span aria-hidden="true" className="portal-home-points-sweep" /><span className="absolute right-3 top-3 grid size-7 place-items-center rounded-full border border-yellow-400/30 text-yellow-300 sm:right-5 sm:top-5 sm:size-9"><BmPointsIcon size={17} /></span><p className="relative pr-7 text-[8px] font-black uppercase tracking-[.15em] text-yellow-400 sm:text-[11px]">Tus puntos</p><p className="relative mt-5 truncate text-xl font-semibold leading-none text-zinc-100 sm:text-3xl"><HomeAnimatedNumber value={data.home.points.total} /></p><span aria-hidden="true" className="portal-home-points-spark portal-home-points-spark-one" /><span aria-hidden="true" className="portal-home-points-spark portal-home-points-spark-two" /><p className="relative mt-2 truncate text-[9px] text-zinc-500 sm:text-xs">+{weeklyPoints} esta semana</p></Link>
  </section>;
}
...
function MonthlyAttendanceIndicator({ data }: { data: PortalData }) {
  ...
  return <Link href="/portal/asistencias" aria-label={`Ver detalle de asistencia mensual. ${percentage === null ? "Sin registros este mes." : `${finalDisplay}. ${detail}.`}`} className="portal-home-interactive group absolute right-0 top-0 z-10 flex w-[6rem] cursor-pointer flex-col items-center gap-1.5 rounded-2xl transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-300 min-[390px]:w-[6.8rem] sm:w-[9rem]" title="Ver detalle de asistencias">
    <span className="relative grid size-[78px] place-items-center rounded-full border border-zinc-700/70 shadow-[0_8px_22px_rgba(0,0,0,.28)] transition group-hover:border-yellow-400/40 min-[390px]:size-[88px] sm:size-[118px]" style={{ background: `conic-gradient(#facc15 ${angle}deg,#27272a 0deg)` }}><span className="absolute inset-[7px] rounded-full bg-[#0b0b0b] sm:inset-[9px]" /><strong className="relative text-lg font-black tabular-nums text-yellow-300 sm:text-2xl">{display}</strong></span>
