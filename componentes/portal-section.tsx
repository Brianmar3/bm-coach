...
function WorkoutView({ data }: { data: PortalData }) {
  ...
  return <>
    <header className="mb-4"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-zinc-500">{routineDisplayName}</p><h1 className="mt-1 text-xl font-black tracking-tight text-white">Día {selectedDay.dayNumber}{selectedDayName ? ` · ${selectedDayName}` : ""}</h1></header>
    ...
    <section className="relative mb-5 overflow-hidden rounded-3xl border border-yellow-400/20 bg-[radial-gradient(circle_at_10%_10%,rgba(250,204,21,.11),transparent_34%),linear-gradient(145deg,#1b1b1b,#0a0a0a)] p-4 shadow-[0_18px_42px_rgba(0,0,0,.34)]">
      ...
      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 pt-7">
        <span aria-hidden="true" className="grid h-12 w-12 place-items-center rounded-2xl border border-yellow-400/20 bg-yellow-400/[.07] text-yellow-300 shadow-[inset_0_0_0_1px_rgba(250,204,21,.03)]"><svg viewBox="0 0 32 32" className="h-7 w-7" fill="none"><path d="M7 12v8m18-8v8M4 14v4m24-4v4M7 16h18" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" /></svg></span>
    ...
    {draft && <>
      ...
      <div className="fixed inset-x-3 bottom-[calc(env(safe-area-inset-bottom)+6rem)] z-30 mx-auto grid max-w-2xl grid-cols-2 gap-2 rounded-2xl border border-yellow-400/15 bg-black/90 p-2 shadow-[0_16px_40px_rgba(0,0,0,.65)] backdrop-blur-xl max-[340px]:grid-cols-1 md:sticky md:bottom-4 md:mt-6 md:flex md:justify-end md:border-zinc-800 md:bg-zinc-950/95"><button type="button" disabled={saving || !started} onClick={() => save(false)} className="min-h-11 min-w-0 rounded-xl border border-yellow-400/40 px-3 py-2.5 text-xs font-bold text-yellow-300 outline-none focus-visible:ring-2 focus-visible:ring-yellow-300 disabled:opacity-50 md:text-sm">{savingAction === "draft" ? "Guardando…" : "Guardar progreso"}</button><button type="button" disabled={saving || !started} onClick={openFinalSummary} className="min-h-11 min-w-0 rounded-xl bg-yellow-400 px-3 py-2.5 text-xs font-black text-zinc-950 outline-none focus-visible:ring-2 focus-visible:ring-yellow-100 disabled:opacity-50 md:text-sm">Finalizar entrenamiento</button></div>
