...
function WorkoutView({ data }: { data: PortalData }) {
  ...
  return <>
    ...
    {draft && <>
      ...
      {hasPersonalizedService(data.profile.serviceType) && <div className="mt-5 border-t border-zinc-800 pt-5"><Link href="/portal/progreso" className="grid min-h-20 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-2xl border border-white/[.08] bg-[linear-gradient(145deg,#171717,#0b0b0b)] p-3.5 outline-none transition hover:border-yellow-400/25 focus-visible:ring-2 focus-visible:ring-yellow-300"><span className="grid size-11 place-items-center rounded-2xl border border-yellow-400/20 bg-yellow-400/[.05] text-yellow-300"><svg viewBox="0 0 24 24" aria-hidden="true" className="size-5 fill-none stroke-current" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M4 19V9m5 10V5m5 14v-7m5 7V3" /><path d="m3 15 5-4 5 2 7-7" /></svg></span><span className="min-w-0"><strong className="block text-sm text-zinc-100">Ver mi progreso</strong><small className="mt-1 block truncate text-zinc-500">Historial, evolución y avances</small></span><span aria-hidden="true" className="text-2xl text-zinc-600">›</span></Link></div>}
