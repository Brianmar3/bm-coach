...
export function QuickLogHistory() {
  ...
  return <div><header className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-yellow-400">Registro personal</p><h1 className="mt-1 text-2xl font-bold">Mi registro</h1><p className="mt-1 text-sm text-zinc-500">Notas, entrenamientos, progresos y fotos.</p></div><button onClick={() => setCreating("NOTE")} className="rounded-xl bg-yellow-400 px-4 py-2.5 text-sm font-bold text-zinc-950">+ Nuevo registro</button></header>
...
function QuickLogCard({ log, edit, remove, removePhoto }: { log: QuickLog; edit: () => void; remove: () => void; removePhoto: (photoId: string) => void }) {
  ...
  return <article className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-wider text-yellow-400">{labels[log.type].icon} {labels[log.type].title}</p><h2 className="mt-1 truncate font-bold">{log.title || labels[log.type].title}</h2><p className="mt-1 text-xs text-zinc-500">{new Date(`${log.date}T12:00:00`).toLocaleDateString("es-AR")}{log.category ? ` · ${log.category}` : ""}</p></div><div className="flex gap-3 text-xs"><button onClick={edit} className="text-yellow-300">Editar</button><button onClick={remove} className="text-red-300">Eliminar</button></div></div>{summary && <p className="mt-3 whitespace-pre-wrap text-sm text-zinc-300">{summary}</p>}{log.mood && <p className="mt-2 text-xs text-zinc-500">Sensación: {log.mood}</p>}{log.hasPain && <p className="mt-2 rounded-lg bg-red-400/10 p-2 text-xs text-red-200">Molestia: {log.painDetails || "Sin detalle"}</p>}{log.photos.length > 0 && <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">{log.photos.map((photo) => <div key={photo.id} className="relative"><img src={photo.blobUrl} alt="Foto del registro" loading="lazy" className="aspect-square w-full rounded-xl object-cover" /><button onClick={() => removePhoto(photo.id)} aria-label="Eliminar foto" className="absolute right-1 top-1 rounded-full bg-black/80 px-2 py-1 text-xs text-red-200">×</button></div>)}</div>}</article>;
}
...
function QuickLogForm({ type, initial, close, saved }: { type: QuickLogType; initial?: QuickLog; close: () => void; saved: () => void | Promise<void> }) {
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/85 p-2 sm:p-5" onMouseDown={(event) => event.target === event.currentTarget && close()}><form onSubmit={submit} className="mx-auto my-2 w-full max-w-xl rounded-2xl border border-zinc-800 bg-zinc-900 p-4 sm:my-8 sm:p-5"><div className="flex justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-yellow-400">Registro rápido</p><h2 className="mt-1 text-xl font-bold">{initial ? "Editar" : "Nuevo"} {labels[type].title.toLowerCase()}</h2></div><button type="button" onClick={close} disabled={saving} className="text-sm text-zinc-400">Cerrar</button></div>{error && <p role="alert" className="mt-3 rounded-lg bg-red-400/10 p-3 text-sm text-red-200">{error}</p>}<div className="mt-4 grid gap-3 sm:grid-cols-2"><Field label="Título opcional"><input value={form.title} onChange={(event) => set("title", event.target.value)} maxLength={120} className="input" /></Field><Field label="Fecha"><input required type="date" value={form.date} onChange={(event) => set("date", event.target.value)} className="input" /></Field>
      ...
      {!initial && (type === "PHOTO" || files.length > 0) && <div className="sm:col-span-2"><input ref={fileInput} type="file" accept="image/jpeg,image/png,image/webp" capture="environment" multiple onChange={(event) => setFiles(Array.from(event.target.files ?? []).slice(0, 4))} className="sr-only" /><button type="button" onClick={() => fileInput.current?.click()} className="rounded-lg border border-yellow-400/30 px-3 py-2 text-sm text-yellow-300">Elegir o tomar fotos</button>{previews.length > 0 && <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">{previews.map((preview) => <img key={preview} src={preview} alt="Vista previa" className="aspect-square rounded-xl object-cover" />)}</div>}</div>}{!initial && type === "PHOTO" && files.length === 0 && <button type="button" onClick={() => fileInput.current?.click()} className="sm:col-span-2 rounded-xl border border-dashed border-yellow-400/40 p-5 text-sm text-yellow-300">Tomar o elegir hasta 4 fotos</button>}
