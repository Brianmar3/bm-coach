...
function QuickLogTypeIcon({ type }: { type: QuickLogType }) {
  if (type === "WORKOUT") return <BmCheckIcon size={16} />;
  if (type === "NOTE") return <BmClipboardIcon size={16} />;
  if (type === "PROGRESS") return <BmProgressIcon size={16} />;
  // BM Icons V1 does not include a photo/camera concept yet.
...
export function QuickLogHistory() {
  ...
  return <div><header className="portal-home-class-row"><p className="text-[11px] font-black uppercase tracking-[.22em] text-yellow-400">Registro personal</p><h1 className="mt-1.5 text-3xl font-black tracking-[-.03em] text-zinc-50">Mis registros</h1><p className="mt-1.5 text-sm text-zinc-400">{logs.length} {logs.length === 1 ? "registro" : "registros"} · {distinctExercises} {distinctExercises === 1 ? "ejercicio" : "ejercicios"}</p>{!creating && <button onClick={() => setCreating(true)} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-xl bg-gradient-to-r from-amber-400 to-yellow-300 px-4 py-2.5 text-sm font-black text-zinc-950 shadow-[0_10px_28px_rgba(250,204,21,.14)] transition active:scale-[.98]"><BmPlusIcon size={17} />Nuevo registro</button>}</header>
    ...
    <div className="portal-home-class-row mt-5 grid grid-cols-2 rounded-xl border border-zinc-800 bg-zinc-950 p-1" style={{ animationDelay: "65ms" }} role="tablist" aria-label="Vista de registros"><button type="button" role="tab" aria-selected={view === "chronological"} onClick={() => setView("chronological")} className={`min-h-10 rounded-lg border text-sm font-semibold transition ${view === "chronological" ? "border-yellow-400/55 bg-yellow-400/[.08] text-yellow-300" : "border-transparent text-zinc-400"}`}>Cronológico</button><button type="button" role="tab" aria-selected={view === "exercises"} onClick={() => setView("exercises")} className={`min-h-10 rounded-lg border text-sm font-semibold transition ${view === "exercises" ? "border-yellow-400/55 bg-yellow-400/[.08] text-yellow-300" : "border-transparent text-zinc-400"}`}>Por ejercicio</button></div>
    <section className="portal-home-class-row mt-3 rounded-2xl border border-zinc-800 bg-zinc-900/80 p-3" style={{ animationDelay: "130ms" }}><div className="flex gap-2"><label className="flex min-h-11 min-w-0 flex-1 items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-950 px-3 text-zinc-500 focus-within:border-yellow-400"><BmSearchIcon size={18} className="shrink-0" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar en mis registros" className="min-w-0 flex-1 bg-transparent text-sm text-zinc-100 outline-none" /></label><span className="grid size-11 shrink-0 place-items-center rounded-xl border border-zinc-700 bg-zinc-950 text-zinc-400"><BmFilterIcon size={18} /></span></div><div className="mt-2 grid grid-cols-1 gap-2 min-[390px]:grid-cols-3"><select value={type} onChange={(event) => setType(event.target.value as QuickLogType | "")} className="min-w-0 rounded-lg border border-zinc-700 bg-zinc-950 px-2 py-2 text-xs"><option value="">Todos los tipos</option>{(Object.keys(labels) as QuickLogType[]).map((value) => <option key={value} value={value}>{labels[value].title}</option>)}</select><input type="date" value={from} onChange={(event) => setFrom(event.target.value)} aria-label="Desde" className="min-w-0 rounded-lg border border-zinc-700 bg-zinc-950 px-2 py-2 text-xs" /><input type="date" value={to} onChange={(event) => setTo(event.target.value)} aria-label="Hasta" className="min-w-0 rounded-lg border border-zinc-700 bg-zinc-950 px-2 py-2 text-xs" /></div></section>
    <div className="mt-4 space-y-3">{loading ? <p className="rounded-xl bg-zinc-900 p-8 text-center text-zinc-500">Cargando registros…</p> : view === "chronological" ? logs.length ? logs.map((log, index) => <QuickLogCard key={log.id} log={log} index={index} edit={() => setEditing(log)} remove={() => remove(log)} removePhoto={(photoId) => removePhoto(log, photoId)} />) : <p className="rounded-xl border border-dashed border-zinc-800 p-8 text-center text-zinc-500">Todavía no hay registros personales.</p> : exerciseGroups.length ? exerciseGroups.map((group) => <section key={group.key} className="portal-home-class-row rounded-2xl border border-yellow-400/10 bg-gradient-to-br from-zinc-900 to-[#0b0b0b] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-bold text-zinc-100">{group.name}</h2><p className="mt-1 text-xs text-zinc-500">{group.logs.length} registro{group.logs.length === 1 ? "" : "s"} · Último: {new Date(`${group.latest.date}T12:00:00`).toLocaleDateString("es-AR")}</p></div><button type="button" onClick={() => setSelectedExercise((value) => value === group.key ? "" : group.key)} className="min-h-10 rounded-lg border border-yellow-400/20 px-3 text-xs font-bold text-yellow-300">{selectedExercise === group.key ? "Ocultar historial" : "Ver historial"}</button></div><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4"><Metric label="Última carga" value={group.latest.currentValue === null ? "Sin carga" : `${group.latest.currentValue.toLocaleString("es-AR")} ${group.latest.unit || "kg"}`} /><Metric label="Máxima histórica" value={`${group.maximum.toLocaleString("es-AR")} ${group.latest.unit || "kg"}`} /><Metric label="Último trabajo" value={group.latest.sets !== null && group.latest.repetitions !== null ? `${group.latest.sets} × ${group.latest.repetitions}` : "Sin datos"} /><Metric label="Última fecha" value={new Date(`${group.latest.date}T12:00:00`).toLocaleDateString("es-AR")} /></div>{selectedExercise === group.key && <div className="mt-4 space-y-3 border-t border-zinc-800 pt-4">{group.logs.map((log, index) => <QuickLogCard key={log.id} log={log} index={index} edit={() => setEditing(log)} remove={() => remove(log)} removePhoto={(photoId) => removePhoto(log, photoId)} />)}</div>}</section>) : <p className="rounded-xl border border-dashed border-zinc-800 p-8 text-center text-zinc-500">Todavía no hay ejercicios registrados.</p>}</div>
    </>}
...
function QuickLogCard({ log, index, edit, remove, removePhoto }: { log: QuickLog; index: number; edit: () => void; remove: () => void; removePhoto: (photoId: string) => void }) {
  ...
  const title = strength ? log.exerciseName : (log.title || labels[log.type].title);
  return <article id={`registro-${log.id}`} className="portal-home-class-row min-w-0 scroll-mt-24 overflow-hidden rounded-2xl border border-yellow-400/15 bg-[radial-gradient(circle_at_top_right,rgba(250,204,21,.08),transparent_34%),linear-gradient(145deg,#171717,#0a0a0a)] p-4 shadow-[0_12px_34px_rgba(0,0,0,.24)]" style={{ animationDelay: `${Math.min(index, 8) * 65 + 195}ms` }}><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl border border-yellow-400/25 bg-yellow-400/[.07] text-yellow-300"><QuickLogTypeIcon type={log.type} /></span><div className="min-w-0"><h2 className="line-clamp-2 break-words font-black text-zinc-100">{title}</h2><p className="mt-1 text-xs text-zinc-500">{new Date(`${log.date}T12:00:00`).toLocaleDateString("es-AR")}{time ? ` · ${time}` : ""}</p></div></div><div className="flex shrink-0 gap-2 text-[11px]"><button onClick={edit} className="inline-flex min-h-9 items-center gap-1 rounded-lg px-1.5 text-yellow-300 focus:outline-none focus:ring-2 focus:ring-yellow-300"><BmEditIcon size={14} />Editar</button><button onClick={remove} className="inline-flex min-h-9 items-center gap-1 rounded-lg px-1.5 text-red-300 focus:outline-none focus:ring-2 focus:ring-red-300"><BmDeleteIcon size={14} />Eliminar</button></div></div>
    {strength && <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3"><div className="rounded-xl border border-white/[.06] bg-black/55 p-3"><p className="text-[9px] font-bold uppercase tracking-wide text-zinc-500">Series × reps</p><p className="mt-1 font-black text-zinc-100">{log.sets} × {log.repetitions}</p></div><div className="rounded-xl border border-white/[.06] bg-black/55 p-3"><p className="text-[9px] font-bold uppercase tracking-wide text-zinc-500">Carga</p><p className="mt-1 font-black text-yellow-300">{log.currentValue === null ? "Peso corporal" : `${log.currentValue.toLocaleString("es-AR")} ${log.unit || "kg"}`}</p></div><div className="col-span-2 rounded-xl border border-white/[.06] bg-black/55 p-3 sm:col-span-1"><p className="text-[9px] font-bold uppercase tracking-wide text-zinc-500">Registro anterior</p><p className="mt-1 text-sm">{log.previousValue === null ? "Sin carga anterior" : `${log.previousSets ?? "—"} × ${log.previousRepetitions ?? "—"} · ${log.previousValue.toLocaleString("es-AR")} ${log.unit || "kg"}`}{difference !== null && <span className="ml-2 text-zinc-500">{difference === 0 ? "Sin cambios" : `${difference > 0 ? "+" : ""}${difference.toLocaleString("es-AR")} ${log.unit || "kg"}`}</span>}</p></div></div>}
    {log.achievements.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{log.achievements.map((achievement) => <span key={achievement.id} className="rounded-full border border-yellow-400/20 bg-yellow-400/[.06] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-yellow-200">{achievement.type === "FIRST_MARK" ? "Primera marca" : achievement.type === "MAX_LOAD" ? "Nueva carga máxima" : achievement.type === "REPETITION_PR" ? "Récord de repeticiones" : `${achievement.recordCount} registros`}</span>)}</div>}{log.content && log.content !== summary && <p className="mt-3 whitespace-pre-wrap break-words text-sm text-zinc-300">{log.content}</p>}{log.feedback && <div className="mt-3 rounded-xl border border-yellow-400/15 bg-yellow-400/[.04] p-3"><p className="text-[10px] font-bold uppercase tracking-wider text-yellow-400">Devolución de {log.feedback.trainerName}</p><p className="mt-1 text-sm text-zinc-200">{log.feedback.text}</p><p className="mt-2 text-[10px] text-zinc-500">{new Date(log.feedback.updatedAt).toLocaleString("es-AR")}</p></div>}{log.mood && <p className="mt-2 text-xs text-zinc-500">Sensación: {log.mood}</p>}{log.hasPain && <p className="mt-2 rounded-lg bg-red-400/10 p-2 text-xs text-red-200">Molestia: {log.painDetails || "Sin detalle"}</p>}{log.photos.length > 0 && <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">{log.photos.map((photo) => <div key={photo.id} className="relative min-w-0 overflow-hidden rounded-xl bg-black"><a href={photo.blobUrl} target="_blank" rel="noreferrer" aria-label="Abrir fotografía en tamaño completo" className="group block focus:outline-none focus:ring-2 focus:ring-yellow-300"><StudentPhoto src={photo.blobUrl} alt="Foto adjunta al registro" loading="lazy" className="aspect-square w-full object-cover transition group-hover:opacity-80" /><span className="absolute inset-x-1 bottom-1 rounded bg-black/75 px-2 py-1 text-center text-[10px] text-zinc-200">Ver completa</span></a><button onClick={() => removePhoto(photo.id)} aria-label="Eliminar foto" className="absolute right-1 top-1 rounded-full bg-black/85 px-2 py-1 text-xs text-red-200 focus:outline-none focus:ring-2 focus:ring-red-300">×</button></div>)}</div>}</article>;
}
