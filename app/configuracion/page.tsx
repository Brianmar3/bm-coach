...
import { allowedLogoModes, BM_DEFAULT_ACCENT, MAX_WORKSPACE_NAME_LENGTH, normalizeAccentColor, normalizeWorkspaceName, resolveWorkspaceBranding, workspaceAccentColor, workspaceBrandingVariables, type WorkspaceBranding, type WorkspaceLogoMode } from "@/lib/workspace-branding";
...
export default function ConfiguracionPage() {
  ...
  function updateBranding(next: Partial<CoachSettings>) { const updated = { ...value, ...next }; setSettings(updated); setSavedSection(null); setError(""); window.dispatchEvent(new CustomEvent(WORKSPACE_BRANDING_EVENT, { detail: updated })); }
  async function submit(event: FormEvent) {
    event.preventDefault();
    const validationError = (value.brandingPlan === "PREMIUM" && !normalizeWorkspaceName(value.systemName) ? "El nombre de tu espacio debe tener entre 1 y 36 caracteres." : null) ?? validateCoachPlans(value.plans) ?? validatePaymentMethods(value.paymentMethods) ?? validateTransferDetails(value.transferDetails);
    if (validationError) { setError(validationError); setSavedSection(null); return; }
    setSaving(true); setError("");
    try { const result = await save([value]); const saved = result.settings ?? value; setSettings(saved); window.dispatchEvent(new CustomEvent(WORKSPACE_BRANDING_EVENT, { detail: saved })); setSavedSection(active); }
  ...
  return <ModuleShell title="Configuración" subtitle="Administrá los datos y preferencias de tu espacio.">
    ...
    <form id="settings-form" onSubmit={submit} className="min-w-0 space-y-4">
      ...
      {active === "personalizacion" && <div className="workspace-brand" style={workspaceBrandingVariables(value.accentColor) as CSSProperties}><Panel title="Personalización" description="Este color identifica tu espacio y se aplica también al portal de todos tus alumnos." icon={<BmSlidersIcon className="size-5" />}>
        <div className="mb-6"><Field label="Nombre de tu espacio"><input value={value.systemName} onChange={(event) => updateBranding({ systemName: event.target.value })} maxLength={MAX_WORKSPACE_NAME_LENGTH} disabled={value.brandingPlan !== "PREMIUM"} className={inputClass} /></Field><p className="mt-1 text-xs text-zinc-500">{value.brandingPlan === "PREMIUM" ? "Se mostrará en tu cabecera y en la de tus alumnos. Máximo 36 caracteres." : "Disponible en PREMIUM. Tu nombre guardado se conserva si cambiás de plan."}</p><p className="mt-2 truncate text-sm font-bold text-yellow-300">Vista previa: {resolveWorkspaceBranding(value, value.brandingPlan).displayName}</p></div>
