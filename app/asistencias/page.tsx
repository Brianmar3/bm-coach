...
function AttendancePageContent() {
  ...
  function markAllPresent() {
    setRoster((current) => current.map((student) => ({ ...student, status: "presente" })));
    void persist(roster.map((student) => ({ studentId: student.id, status: "presente" as const })));
  }
  ...
  return (
    <ModuleShell title="" subtitle="" hideHeader flushTop>
      ...
      <section className={`${historyMode ? "hidden " : ""}rounded-2xl border border-zinc-800 bg-zinc-900`}>
        ...
        <div className="border-b border-zinc-800 p-4"><label className="text-sm">Buscar alumno<input value={studentQuery} onChange={(event) => setStudentQuery(event.target.value)} placeholder="Nombre, apellido o teléfono" className={`${inputClass} mt-1`} /></label>{calendarMode && studentQuery.trim() && <div className="mt-2 overflow-hidden rounded-xl border border-zinc-700 bg-zinc-950">{!ready ? <p className="p-3 text-sm text-zinc-500">Buscando alumnos…</p> : addableStudents.length ? addableStudents.map((student) => <button key={student.id} onClick={() => addExceptional(student)} className="flex w-full items-center justify-between gap-3 border-b border-zinc-800 px-3 py-2.5 text-left text-sm last:border-0 hover:bg-zinc-900"><span>{student.firstName} {student.lastName}</span><span className="text-xs text-zinc-500">{student.phone}</span></button>) : <p className="p-3 text-sm text-zinc-500">No se encontraron alumnos activos.</p>}</div>}</div>
        ...
        {!ready ? <p className="p-12 text-center text-zinc-500">Cargando alumnos…</p> : loadingRoster ? <p className="p-12 text-center text-zinc-500">Cargando alumnos…</p> : rosterStudents.length === 0 ? <p className="p-12 text-center text-zinc-500">No hay alumnos para esta fecha. Podés agregar excepciones con el buscador.</p> : <div className="grid gap-3 p-3 sm:p-4">{rosterStudents.map((student) => <AttendanceStudentCard key={student.id} student={student} highlighted={student.id === targetStudentId} setStatus={(status) => setStatus(student.id, status)} />)}</div>}
        ...
        <div className="flex justify-end border-t border-zinc-800 p-4"><button onClick={save} disabled={saving || !date} className="w-full rounded-xl bg-yellow-400 px-5 py-3 font-bold text-zinc-950 disabled:opacity-50 sm:w-auto">{saving ? "Guardando…" : "Guardar"}</button></div>
      </section>
...
function AttendanceStudentCard({ student, highlighted, setStatus }: { student: AttendanceRosterStudent; highlighted: boolean; setStatus: (status: AttendanceStatus) => void }) { return <article className={`scroll-mt-24 rounded-xl border p-3 ${highlighted ? "border-yellow-300 bg-yellow-400/10 shadow-[0_0_24px_rgba(250,204,21,.08)]" : student.assigned ? "border-zinc-800 bg-zinc-950" : "border-yellow-400/30 bg-yellow-400/5"}`}><div className="flex items-center justify-between gap-3"><h3 className="truncate text-sm font-semibold">{student.name}{highlighted ? <span className="ml-2 text-[10px] font-bold uppercase tracking-wide text-yellow-300">Respondió</span> : null}</h3>{student.status ? <span className="shrink-0 rounded-full bg-zinc-800 px-2 py-1 text-[10px] text-zinc-300">{STATUS_LABEL[student.status]}</span> : <span className="shrink-0 rounded-full border border-zinc-700 px-2 py-1 text-[10px] text-zinc-500">Sin registrar</span>}</div><div className="mt-2 grid grid-cols-3 gap-2">{STATUS_OPTIONS.map((status) => <button key={status} onClick={() => setStatus(status)} className={`min-h-10 rounded-lg border px-2 py-2 text-xs font-semibold transition ${student.status === status ? status === "presente" ? "border-emerald-400 bg-emerald-400/15 text-emerald-300" : status === "ausente" ? "border-red-400 bg-red-400/15 text-red-300" : "border-yellow-400 bg-yellow-400/15 text-yellow-300" : "border-zinc-700 text-zinc-400 hover:border-zinc-500"}`}>{STATUS_LABEL[status]}</button>)}</div></article>; }
