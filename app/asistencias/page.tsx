...
export default function AsistenciasPage() {
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    fetch(`/api/asistencias?date=${date}&scheduleId=${encodeURIComponent(effectiveScheduleId)}`, { cache: "no-store", signal: controller.signal })
      .then(async (response) => { if (!response.ok) throw new Error(await responseError(response, "No se pudo cargar el grupo.")); return response.json() as Promise<AttendanceRoster>; })
  ...
  const addableStudents = useMemo(() => {
    ...
    return students.filter((student) => student.status === "activo" && !existing.has(student.id) && `${student.firstName} ${student.lastName} ${student.phone}`.toLocaleLowerCase("es").includes(normalized)).slice(0, 6);
  ...
  function addExceptional(student: Student) { setRoster((current) => [...current, { id: student.id, name: `${student.firstName} ${student.lastName}`.trim(), phone: student.phone, assigned: false, status: null, attendanceId: null }].sort((left, right) => left.name.localeCompare(right.name, "es"))); setStudentQuery(""); }
  function changeDate(value: string) { setDate(value); setLoadingRoster(true); setError(""); setSaved(false); }
  function changeSchedule(value: string) { setScheduleId(value); setLoadingRoster(true); setError(""); setSaved(false); }

  async function save() {
    const records = roster.filter((student) => student.status).map((student) => ({ studentId: student.id, status: student.status }));
    if (!effectiveScheduleId) { setError("Seleccioná un horario para registrar la asistencia."); return; }
    if (!records.length) { setError("Marcá al menos un alumno como presente, ausente o justificado."); return; }
  ...
  return (
    <ModuleShell title="Asistencias" subtitle="Registrá presentes, ausentes y justificados por cada horario semanal." action={<button onClick={save} disabled={saving || !effectiveScheduleId} className="rounded-xl bg-yellow-400 px-4 py-3 font-bold text-zinc-950 disabled:opacity-50">{saving ? "Guardando…" : "Guardar asistencias"}</button>}>
      ...
      <section className="rounded-2xl border border-zinc-800 bg-zinc-900">
        <div className="grid gap-4 border-b border-zinc-800 p-4 md:grid-cols-[220px_1fr_auto] md:items-end"><label className="text-sm">Fecha<input type="date" value={date} onChange={(event) => changeDate(event.target.value)} className={`${inputClass} mt-1`} /></label><label className="text-sm">Clase, horario o grupo<select value={effectiveScheduleId} onChange={(event) => changeSchedule(event.target.value)} className={`${inputClass} mt-1`}><option value="">Seleccionar horario</option>{dateSchedules.map((schedule) => <option key={schedule.id} value={schedule.id}>{scheduleLabel(schedule)}</option>)}</select></label><button onClick={markAllPresent} disabled={!roster.length} className="rounded-xl border border-yellow-400/40 px-4 py-2.5 text-sm font-bold text-yellow-300 disabled:opacity-40">Marcar todos presentes</button></div>
        {!ready ? <p className="p-12 text-center text-zinc-500">Cargando alumnos…</p> : !effectiveScheduleId ? <p className="p-12 text-center text-zinc-500">No hay un horario configurado para el día seleccionado.</p> : loadingRoster ? <p className="p-12 text-center text-zinc-500">Cargando alumnos…</p> : roster.length === 0 ? <p className="p-12 text-center text-zinc-500">Este horario todavía no tiene alumnos activos asignados.</p> : <div className="grid gap-3 p-4 lg:grid-cols-2">{roster.map((student) => <AttendanceStudentCard key={student.id} student={student} setStatus={(status) => setStatus(student.id, status)} />)}</div>}
        {effectiveScheduleId && <div className="border-t border-zinc-800 p-4"><h3 className="font-semibold">Agregar alumno de otro horario</h3><p className="mt-1 text-xs text-zinc-500">La excepción se registra solo para esta fecha y no cambia su grupo habitual.</p><div className="relative mt-3 max-w-xl"><input value={studentQuery} onChange={(event) => setStudentQuery(event.target.value)} placeholder="Buscar por nombre o teléfono" className={inputClass} />{studentQuery && <div className="absolute z-10 mt-1 w-full overflow-hidden rounded-xl border border-zinc-700 bg-zinc-950 shadow-xl">{addableStudents.length ? addableStudents.map((student) => <button key={student.id} onClick={() => addExceptional(student)} className="flex w-full items-center justify-between border-b border-zinc-800 px-4 py-3 text-left text-sm last:border-0 hover:bg-zinc-900"><span>{student.firstName} {student.lastName}</span><span className="text-xs text-zinc-500">{student.phone}</span></button>) : <p className="p-4 text-sm text-zinc-500">No se encontraron alumnos activos.</p>}</div>}</div></div>}
      </section>
