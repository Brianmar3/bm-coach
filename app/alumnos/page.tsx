...
import { bmiFromCentimeters } from "@/lib/height";
...
function bmi(weight: number, height: number) { return bmiFromCentimeters(weight, height); }
...
export default function AlumnosPage() {
    ...
    const [chooseEnrollment, setChooseEnrollment] = useState(false);
    ...
    async function remove(item: Student) {
        if (!window.confirm(`¿Dar de baja a ${item.firstName} ${item.lastName}?\n\nDejará de aparecer en la gestión activa y no recibirá nueva planificación. Su historial se conservará y podrás reactivarlo editando su estado.`))
            return;
        setError("");
        try {
            const response = await fetch(`/api/alumnos/${item.id}`, { method: "DELETE" });
            if (!response.ok)
                throw new Error(await responseError(response, "No se pudo dar de baja al alumno."));
            const result = await response.json() as { student: Student };
            setItems((current) => current.map((student) => student.id === item.id ? result.student : student));
            if (viewing?.id === item.id)
                setViewing(null);
        }
        catch (deleteError) {
            setError(deleteError instanceof Error ? deleteError.message : "No se pudo dar de baja al alumno.");
        }
    }
    if (!ready)
    ...
    <div id="student-results">{visible.length === 0 ? <div role="status" aria-live="polite" className="px-4 py-8 text-center"><p className="text-sm font-semibold text-zinc-300">{hasAppliedSearch ? <>No encontramos alumnos para “{appliedQuery.trim()}”.</> : hasNonSearchFilters ? "No encontramos alumnos con estos filtros." : "Todavía no tenés alumnos."}</p><p className="mt-1 text-xs text-zinc-500">{hasAppliedSearch ? "Podés corregir la búsqueda sin salir de esta pantalla." : hasNonSearchFilters ? "Probá cambiar los filtros activos." : "Agregá el primero para empezar a gestionar su plan y seguimiento."}</p>{hasNonSearchFilters && <button type="button" onClick={clearFilters} className="mt-3 min-h-11 px-4 text-sm font-bold text-yellow-300">Limpiar filtros</button>}{!hasAppliedSearch && !hasNonSearchFilters && <button type="button" onClick={() => setChooseEnrollment(true)} className="mt-3 min-h-11 px-4 text-sm font-bold text-yellow-300">Agregar alumno</button>}</div> : <><div className="space-y-2 p-2 pb-20 md:hidden">{visible.map((item) => <StudentMobileCard key={item.id} item={item} view={() => setViewing(item)} edit={() => void begin(item)} remove={() => void remove(item)}/>)}</div><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[960px] text-left text-sm"><thead className="text-zinc-500"><tr><th className="p-4">Alumno</th><th>Servicio</th><th>Plan</th><th>Horario principal</th><th>Contacto</th><th>Vencimiento</th><th>Estado</th><th aria-label="Acciones"/></tr></thead><tbody>{visible.map((item) => <tr key={item.id} className="border-t border-zinc-800"><td className="p-4 font-medium">{item.firstName} {item.lastName}<span className="block text-xs font-normal text-zinc-500">{item.studentType} · IMC {bmi(item.weight, item.height)} · {age(item.birthDate)} años</span></td><td><ServiceBadge value={item.serviceType}/></td><td>{item.plan}<span className="block text-xs text-zinc-500">{money(item.monthlyFee)}</span></td><td className="max-w-56 text-xs text-zinc-400">{item.scheduleLabel ?? "Sin horario principal"}</td><td>{item.studentType === "Kids" ? item.responsiblePhone || item.phone || "Sin teléfono" : item.phone}<span className="block text-xs text-zinc-500">{item.email || "Sin correo"}</span></td><td>{showDate(item.dueDate)}</td><td><span className={`rounded-full px-2 py-1 text-xs font-bold capitalize ${item.status === "activo" ? "bg-emerald-400/15 text-emerald-300" : "bg-zinc-700 text-zinc-300"}`}>{item.status}</span></td><td className="space-x-3 whitespace-nowrap pr-4 text-yellow-400"><button onClick={() => setViewing(item)}>Ver ficha</button><button onClick={() => begin(item)}>Editar</button><button onClick={() => remove(item)} className="text-red-300">Dar de baja</button></td></tr>)}</tbody></table></div></>}</div></section>
    <StudentInvitations open={inviteOpen} onClose={() => setInviteOpen(false)} />
    {chooseEnrollment && <div className="fixed inset-0 z-[70] grid place-items-center bg-black/85 p-4"><section role="dialog" aria-modal="true" aria-label="Nuevo alumno" className="w-full max-w-md rounded-2xl border border-zinc-700 bg-zinc-900 p-5 text-white"><div className="flex items-center justify-between"><h2 className="text-xl font-black">Nuevo alumno</h2><button type="button" onClick={() => setChooseEnrollment(false)} className="p-2 text-zinc-400">Cerrar</button></div><div className="mt-5 grid gap-3"><button type="button" onClick={() => { setChooseEnrollment(false); setInviteOpen(true); }} className="rounded-xl border border-yellow-400/50 bg-yellow-400/10 p-4 text-left"><strong className="block text-yellow-300">Invitar alumno</strong><span className="mt-1 block text-sm text-zinc-400">El alumno completa sus datos desde un enlace.</span></button><button type="button" onClick={() => { setChooseEnrollment(false); void begin(); }} className="rounded-xl border border-zinc-700 p-4 text-left"><strong className="block">Cargar manualmente</strong><span className="mt-1 block text-sm text-zinc-400">Completá vos los datos del alumno.</span></button></div></section></div>}
    ...
    <TrainerFloatingActions mode="direct" enabled={!open && !viewing && !chooseEnrollment && !inviteOpen} actions={[{ label: "Nuevo alumno", symbol: "+", onSelect: () => setChooseEnrollment(true) }]} />
  </ModuleShell>;
}

function StudentMobileCard({ item, view, edit, remove }: { item: Student; view: () => void; edit: () => void; remove: () => void }) {
...
function MobileDatum({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
    return <div className="flex min-w-0 items-start gap-2"><span className="mt-0.5 shrink-0 text-yellow-400/70">{icon}</span><span className="min-w-0"><span className="block text-[9px] uppercase tracking-wide text-zinc-600">{label}</span><span title={value} className="block truncate text-[11px] text-zinc-300">{value}</span></span></div>;
}

function MobileAction({ icon, label, action, danger = false }: { icon: React.ReactNode; label: string; action: () => void; danger?: boolean }) {
    return <button type="button" onClick={action} className={`inline-flex min-h-11 min-w-0 items-center justify-center gap-1 border-r border-zinc-800/80 px-1 text-[10px] font-bold last:border-r-0 ${danger ? "text-red-300" : "text-yellow-300"}`}>{icon}<span className="truncate">{label}</span></button>;
}
