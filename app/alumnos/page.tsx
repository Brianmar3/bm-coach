...
function blank(options: EnrollmentOptions, previous?: Pick<StudentFormValue, "plan" | "joinedAt" | "scheduleId" | "status" | "studentType" | "responsibleName" | "responsiblePhone" | "responsibleRelation">): StudentFormValue {
  ...
  return { firstName: "", lastName: "", phone: "", email: "", birthDate: "", weight: 0, height: 0, goal: "", plan: plan?.name ?? "2 días por semana", monthlyFee: plan?.price ?? 0, joinedAt, dueDate: nextMonthlyDate(joinedAt), status: previous?.status ?? "activo", notes: "", studentType: previous?.studentType ?? "Adulto", responsibleName: previous?.responsibleName ?? "", responsiblePhone: previous?.responsiblePhone ?? "", responsibleRelation: previous?.responsibleRelation ?? "", scheduleId };
}
