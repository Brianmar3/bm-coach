...
function StudentDetail({ item, close }: { item: Student; close: () => void }) { return <div className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-4"><section className="w-full max-w-2xl rounded-2xl bg-zinc-900 p-6 text-white"><div className="flex justify-between"><div><h2 className="text-xl font-bold">{item.firstName} {item.lastName}</h2><p className="text-sm text-zinc-400">{item.plan} · {item.status}</p></div><button onClick={close} className="text-zinc-400">Cerrar</button></div><dl className="mt-5 grid gap-4 text-sm sm:grid-cols-3"><div><dt className="text-zinc-500">Edad / IMC</dt><dd>{age(item.birthDate)} años · {bmi(item.weight, item.height)}</dd></div><div><dt className="text-zinc-500">Objetivo</dt><dd>{item.goal || "No definido"}</dd></div><div><dt className="text-zinc-500">Cuota</dt><dd>{money(item.monthlyFee)}</dd></div><div><dt className="text-zinc-500">Contacto</dt><dd>{item.phone}</dd></div><div><dt className="text-zinc-500">Vencimiento</dt><dd>{new Date(`${item.dueDate}T12:00:00`).toLocaleDateString("es-AR")}</dd></div><div><dt className="text-zinc-500">Historial básico</dt><dd>Ficha creada el {new Date(`${item.joinedAt}T12:00:00`).toLocaleDateString("es-AR")}</dd></div></dl><p className="mt-5 rounded-xl bg-zinc-950 p-4 text-sm text-zinc-300">{item.notes || "Sin observaciones."}</p><div className="mt-5 flex flex-wrap gap-3"><Link href="/pagos" className="rounded-lg bg-yellow-400 px-3 py-2 text-sm font-bold text-zinc-950">Registrar pago</Link><Link href="/rutinas" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm text-yellow-400">Ver rutinas</Link><Link href="/evaluaciones" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm text-yellow-400">Ver evaluaciones</Link><Link href="/clases" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm text-yellow-400">Ver clases</Link></div></section></div>; }
