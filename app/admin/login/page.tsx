...
export default function AdminLoginPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (loading) return;
    if (legacy && (!token.trim() || token.trim().length < 32)) {
      setFieldError(token.trim() ? "Revisá que la credencial esté completa." : "Ingresá tu credencial administrativa.");
      tokenRef.current?.focus();
      return;
    }
    if (!legacy && (!email.trim() || !password)) { setFieldError("Ingresá tu email y contraseña."); return; }
    setLoading(true);
    setError("");
    try {
      const response = await fetch("/api/admin/auth/login", {
        ...
        body: JSON.stringify(legacy ? { token } : { email, password }),
      });
  ...
  return <main className="grid min-h-screen place-items-center bg-zinc-950 p-5 text-white">
    <form noValidate onSubmit={submit} className="w-full max-w-md rounded-3xl border border-yellow-400/20 bg-zinc-900 p-7 shadow-2xl shadow-black">
      ...
      {legacy ? <div className="mt-6"><PasswordField ref={tokenRef} id="admin-credential" label="Credencial administrativa" required minLength={32} autoComplete="current-password" value={token} error={fieldError} onChange={(event) => { setToken(event.target.value); setError(""); if (fieldError) setFieldError(""); }} className="bg-black" /></div> : <div className="mt-6 space-y-4"><label htmlFor="trainer-email" className="block text-sm font-medium text-zinc-200">Email<input id="trainer-email" type="email" required autoComplete="username" value={email} onChange={(event) => { setEmail(event.target.value); setFieldError(""); }} className="mt-2 min-h-12 w-full rounded-xl border border-zinc-700 bg-black px-3 text-white outline-none focus:border-yellow-400"/></label><PasswordField id="trainer-password" label="Contraseña" required autoComplete="current-password" value={password} error={fieldError} onChange={(event) => { setPassword(event.target.value); setError(""); if (fieldError) setFieldError(""); }} className="bg-black" /></div>}
      ...
      <button type="button" onClick={() => { setLegacy((value) => !value); setFieldError(""); setError(""); }} className="mt-3 min-h-11 w-full text-sm text-zinc-500 hover:text-yellow-300">{legacy ? "Ingresar con email" : "Usar credencial administrativa histórica"}</button>
