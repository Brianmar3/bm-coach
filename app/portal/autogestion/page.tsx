...
export default async function SelfServiceHomePage() {
  ...
  return <SelfServiceShell student={student}><div className="portal-home-sequence mx-auto max-w-5xl space-y-4">
    ...
    <PortalRoutineFrame><p className="text-[9px] font-black uppercase tracking-[.2em] text-yellow-400">Tu rutina</p><div className="mt-2.5 flex items-center gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-full border border-yellow-400/25 bg-yellow-400/[.05] text-yellow-300"><BmRoutineIcon size={22} /></span><h2 className="text-base font-black leading-tight text-zinc-50 min-[390px]:text-lg">Todavía no creaste una rutina.</h2></div><Link href="/portal/autogestion/rutina" className="portal-home-interactive mt-2.5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl border border-yellow-400/45 px-4 text-sm font-bold text-yellow-300 transition hover:bg-yellow-400/[.06]">Crear mi rutina <span aria-hidden="true">→</span></Link></PortalRoutineFrame>
