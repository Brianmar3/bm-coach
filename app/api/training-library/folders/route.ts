...
export async function GET() {
  ...
  const folders = await prisma.trainingLibraryFolder.findMany({ where: accessibleContentWhere((await requireTrainerWorkspace()).workspaceId), include: { _count: { select: { blockTemplates: true } } }, orderBy: { name: "asc" } });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const folder = await prisma.trainingLibraryFolder.create({ data: { workspaceId, scope: "WORKSPACE", name: name!.trim(), normalizedName: normalizeLibraryText(name!) }, include: { _count: { select: { blockTemplates: true } } } });
