...
export async function GET() {
  ...
  const records = await prisma.trainingBlockTemplate.findMany({ where: accessibleContentWhere(workspaceId), include, orderBy: { updatedAt: "desc" } });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const record = await prisma.$transaction(async (transaction) => {
      ...
      return transaction.trainingBlockTemplate.create({ data: { workspaceId, scope: "WORKSPACE", name: clean.name, type: clean.block.type, content: clean.block as unknown as Prisma.InputJsonValue, folderId: clean.folderId || null, tags: { create: tags.map((tag) => ({ tagId: tag.id })) } }, include });
    });
