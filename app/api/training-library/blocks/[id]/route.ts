...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const record = await prisma.$transaction(async (transaction) => {
      if (clean.folderId) {
        const folder = await transaction.trainingLibraryFolder.findFirst({ where: { id: clean.folderId, workspaceId, scope: "WORKSPACE", status: "ACTIVE" }, select: { id: true } });
        if (!folder) throw new Error("FOLDER_NOT_FOUND");
      }
      const tags = await Promise.all(normalizedLibraryTags(clean.tags).map((tag) => transaction.trainingLibraryTag.upsert({ where: { workspaceId_normalizedName: { workspaceId, normalizedName: tag.normalizedName } }, update: {}, create: { ...tag, workspaceId, scope: "WORKSPACE" }, select: { id: true } })));
      await transaction.trainingBlockTemplateTag.deleteMany({ where: { blockTemplateId: id } });
      return transaction.trainingBlockTemplate.update({ where: { id, workspaceId, scope: "WORKSPACE" }, data: { name: clean.name, type: clean.block.type, content: clean.block as unknown as Prisma.InputJsonValue, folderId: clean.folderId || null, tags: { create: tags.map((tag) => ({ tagId: tag.id })) } }, include });
    });
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (typeof isFavorite === "boolean") {
      const active = await prisma.trainingBlockTemplate.findFirst({ where: { id, workspaceId, scope: "WORKSPACE", status: "ACTIVE" }, select: { id: true } });
      if (!active) return Response.json({ error: "El bloque no está activo o ya no existe." }, { status: 404 });
      const record = await prisma.trainingBlockTemplate.update({ where: { id, workspaceId, scope: "WORKSPACE" }, data: { isFavorite }, include });
      return Response.json(serializeLibraryBlock(record));
    }
    if (action === "markUsed") {
      const active = await prisma.trainingBlockTemplate.findFirst({ where: { id, workspaceId, scope: "WORKSPACE", status: "ACTIVE" }, select: { id: true } });
      const updated = active ? await prisma.trainingBlockTemplate.update({ where: { id, workspaceId, scope: "WORKSPACE" }, data: { lastUsedAt: new Date() }, include }) : null;
    ...
    const record = await prisma.trainingBlockTemplate.update({ where: { id, workspaceId, scope: "WORKSPACE" }, data: { status: archived ? "ARCHIVED" : "ACTIVE", archivedAt: archived ? new Date() : null }, include });
    return Response.json(serializeLibraryBlock(record));
  } catch (error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2025") return Response.json({ error: "El bloque ya no existe." }, { status: 404 });
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const removed = await prisma.$transaction(async (transaction) => {
      const block = await transaction.trainingBlockTemplate.findUnique({ where: { id, workspaceId, scope: "WORKSPACE" }, select: { id: true, name: true } });
