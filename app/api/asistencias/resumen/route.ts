...
export async function GET(request: Request) {
  try {
    ...
    const [todayRecords, monthlyRecords, absenceGroups] = await Promise.all([
      prisma.classAttendance.groupBy({ by: ["status"], where: { student: { workspaceId }, date: { gte: date, lt: nextDay } }, _count: { _all: true } }),
      prisma.classAttendance.groupBy({ by: ["status"], where: { student: { workspaceId }, date: { gte: range.start, lt: range.end } }, _count: { _all: true } }),
      prisma.classAttendance.groupBy({ by: ["studentId"], where: { student: { workspaceId }, status: "ABSENT", date: { gte: recentStart, lte: date } }, _count: { _all: true }, having: { studentId: { _count: { gte: 2 } } }, orderBy: { _count: { studentId: "desc" } }, take: 8 }),
    ]);
    const students = absenceGroups.length ? await prisma.studentRecord.findMany({ where: { workspaceId, id: { in: absenceGroups.map((group) => group.studentId) } }, select: { id: true, data: true } }) : [];
