...
function RoutineEditor({ form, setForm, students, activeDay, setActiveDay, error, close, submit, editingStatus, saving }: { form: RoutineDraft; setForm: (form: RoutineDraft) => void; students: Student[]; activeDay: number; setActiveDay: (day: number) => void; error: string; close: () => void; submit: (event: FormEvent) => void; editingStatus: TrainingRoutineStatus | null; saving: boolean }) {
  ...
  const currentDayIndex = form.days.findIndex((day) => day.clientId === currentDay.clientId);
  ...
  const dayNavigation = <div className="mt-3 flex items-center justify-between gap-3"><button type="button" disabled={currentDayIndex <= 0} onClick={() => setActiveDay(form.days[currentDayIndex - 1].dayNumber)} className="min-h-10 rounded-lg border border-zinc-700 px-3 text-xs font-bold text-zinc-300 disabled:opacity-35">← Día anterior</button><span className="text-xs text-zinc-500">Día {currentDayIndex + 1} de {form.days.length}</span><button type="button" disabled={currentDayIndex < 0 || currentDayIndex >= form.days.length - 1} onClick={() => setActiveDay(form.days[currentDayIndex + 1].dayNumber)} className="min-h-10 rounded-lg border border-zinc-700 px-3 text-xs font-bold text-zinc-300 disabled:opacity-35">Día siguiente →</button></div>;
