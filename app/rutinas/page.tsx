...
import { RoutineAIProposalPanel } from "@/componentes/routine-ai-proposal";
...
import type { RoutineAIDay, RoutineAIProposal } from "@/types/routine-ai";
...
function RoutineEditor({ form, setForm, students, activeDay, setActiveDay, error, close, submit, editingStatus, saving }: { form: RoutineDraft; setForm: (form: RoutineDraft) => void; students: Student[]; activeDay: number; setActiveDay: (day: number) => void; error: string; close: () => void; submit: (event: FormEvent) => void; editingStatus: TrainingRoutineStatus | null; saving: boolean }) {
  ...
  function applyAIProposal(days: RoutineAIDay[], proposal: RoutineAIProposal) {
    const applied: DayDraft[] = days.map((day, dayIndex) => ({ id: undefined, clientId: crypto.randomUUID(), dayNumber: dayIndex + 1, name: day.name, objective: day.focus, warmup: day.warmup, observations: day.notes, estimatedMinutes: day.estimatedMinutes, exercises: [], blocks: day.blocks.map((block, blockIndex) => ({ id: undefined, clientId: crypto.randomUUID(), type: block.type, name: block.name, order: blockIndex + 1, rounds: block.rounds, durationSeconds: block.durationSeconds, workSeconds: block.workSeconds, restSeconds: block.restSeconds, restBetweenRoundsSeconds: block.restBetweenRoundsSeconds, targetRounds: block.targetRounds, instructions: block.instructions, exercises: block.exercises.map((exercise, exerciseIndex) => ({ clientId: crypto.randomUUID(), name: exercise.name, muscleGroup: exercise.muscleGroup, sets: exercise.sets, repetitions: exercise.repetitions, weight: null, effortType: exercise.effortType, effortValue: exercise.effortValue, restSeconds: exercise.restSeconds, observations: exercise.observations, videoUrl: "", tempo: "", alternativeExercise: exercise.alternativeExercise, equipment: exercise.equipment, optional: false, targetType: exercise.targetType, targetSeconds: exercise.targetSeconds, targetRepetitions: exercise.targetRepetitions, targetDistance: exercise.targetDistance, targetSide: exercise.targetSide, order: exerciseIndex + 1 })) })) }));
    if (!applied.length) return; setForm({ ...form, description: form.description || proposal.summary, days: applied }); setActiveDay(1);
  }
  return <div className="fixed inset-0 z-50 overflow-auto bg-black/80 p-0 sm:p-3"><form onSubmit={submit} className="mx-auto min-h-dvh w-full max-w-7xl border border-zinc-800 bg-zinc-900 p-3 text-white sm:my-4 sm:min-h-0 sm:rounded-2xl sm:p-5">
    ...
    {form.kind === "assigned" && <StudentAssignmentPicker students={students} selectedIds={form.studentIds} toggle={toggleStudent} />}
    {form.kind === "assigned" && <RoutineEvaluationPanel student={selectedStudent} context={evaluationContext} loading={evaluationLoading} />}
    {form.kind === "assigned" && <RoutineAIProposalPanel student={selectedStudent} evaluationContext={evaluationContext} objective={form.objective} level={form.level} location={form.location} equipment={form.equipment} onApply={applyAIProposal} />}
...
function StudentAssignmentPicker({ students, selectedIds, toggle }: { students: Student[]; selectedIds: string[]; toggle: (studentId: string) => void }) {
  const [query, setQuery] = useState("");
  ...
  return <fieldset className="mt-6 min-w-0"><legend className="font-semibold text-yellow-400">Alumnos asignados</legend>{selected.length > 0 && <div aria-label="Alumnos seleccionados" className="mt-3 flex min-w-0 flex-wrap gap-2">{selected.map((student) => <span key={student.id} className="inline-flex max-w-full items-center gap-2 rounded-lg border border-yellow-400/40 bg-yellow-400/10 px-3 py-2 text-sm text-yellow-200"><span className="truncate">{student.firstName} {student.lastName}</span><button type="button" aria-label={`Quitar a ${student.firstName} ${student.lastName}`} onClick={() => toggle(student.id)} className="grid size-6 shrink-0 place-items-center rounded-md text-zinc-400 hover:bg-zinc-800 hover:text-white">×</button></span>)}</div>}<label className="mt-3 block text-sm text-zinc-400">Buscar alumno<input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Nombre o apellido" className={`${inputClass} mt-1`} /></label><div className="mt-2 min-w-0 overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950/70">{!query.trim() ? <p className="p-4 text-center text-sm text-zinc-500">Escribí para buscar alumnos</p> : matches.length ? matches.map((student) => <button key={student.id} type="button" onClick={() => { toggle(student.id); setQuery(""); }} className="flex min-h-11 w-full min-w-0 items-center border-b border-zinc-800 px-3 py-2.5 text-left text-sm last:border-0 hover:bg-zinc-800"><span className="truncate">{student.firstName} {student.lastName}</span></button>) : <p className="p-4 text-center text-sm text-zinc-500">No se encontraron alumnos</p>}</div></fieldset>;
}
