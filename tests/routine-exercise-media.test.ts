...
const routineOverlay = readFileSync(new URL("../componentes/routine-overlay.tsx", import.meta.url), "utf8");
...
test("el alumno muestra detalle seguro, instrucciones y atribución sin duplicar video", () => {
  ...
  assert.match(mediaComponent, /<RoutineOverlay open=\{open\}/);
  assert.match(mediaComponent, /aria-label="Cerrar video"/);
  assert.match(mediaComponent, /<BmCloseIcon size=\{22\} \/>/);
  assert.match(routineOverlay, /createPortal/);
  assert.match(routineOverlay, /document\.body/);
  assert.match(routineOverlay, /z-\[210\]/);
  assert.match(routineOverlay, /safe-area-inset-top/);
  assert.match(routineOverlay, /safe-area-inset-bottom/);
  assert.match(routineOverlay, /max-w-lg/);
